...
check_if_couchdb_cluster_is_ready() {
	wait_count=0
	until curl -s -u "$COUCHDB_USER:$COUCHDB_PASSWORD" "http://$SVC_NAME:5984/_cluster_setup" | grep -q "single_node_enabled"; do
		echo "Waiting for cht couchdb cluster to be set up" >&2
		wait_count=$((wait_count + 1))
		curl -u "$COUCHDB_USER:$COUCHDB_PASSWORD" "http://$SVC_NAME:5984/_cluster_setup" || :
		curl -u "$COUCHDB_USER:$COUCHDB_PASSWORD" "http://$SVC_NAME:5984/_membership" || :
