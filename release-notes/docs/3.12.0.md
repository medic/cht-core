...
CHT-Core is collecting new telemetry data:
...
The Monitoring API version 1 (`/api/v1/monitoring`) is now deprecated and the version 2 (`/api/v2/monitoring`) is introduced with new metrics available:
...
CHT-Core will send a SMS when the origin phone number is not registered in the app:
