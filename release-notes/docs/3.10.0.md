...
- [Improved performance of processing deleted docs](#improved-performance-of-processing-deleted-docs)
- [And more...](#and-more)
...
Supporting remote first-time login is only fully functional while using the latest release of medic-android.
// todo version number
...
3.10.0 adds a new authentication mechanism that doesn't require users to know their username or password. Users can now access the app for the first time by clicking on a token authentication link that they receive by SMS. Admins can enable or disable token authentication for any user.   
This feature requires the latest version of the `medic-android` app to function fully, because it enables devices to be prompted to open CHT deployment weblinks with the `medic-android` app.  
See [the documentation](https://docs.communityhealthtoolkit.org/apps/reference/api/#login-by-sms) for more information.    
![token login](./images/3.10-token-login.png)
...
| Version | Number of users | Number of deleted docs | Time to process |
| ------- | --------------- | ---------------------- | --------------- |
| 3.9.0 | 1000 | 500 | 60 minutes | 
| 3.10.0 | 1000 | 500 | 45 seconds |  
| 3.10.0 | 1000 | 5000 | 11 minutes |  
