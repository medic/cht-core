...
- [Complete reference configuration](#complete-reference-configuration)
- [Optimised form HTML generation](#optimised-form-html-generation)
- [Purging server-side](#purging-server-side)
...
possibilities: for example having multiple person types or unbalanced place trees. 
...
Version 3.7.0 includes a complete reference configuration that is enabled automatically 
on fresh application installations. It provides a foundation for forms, data fields, 
and analytics and has been designed to model an ideal way of structuring and organizing 
a workflow, its configuration code, and testing framework.

## Optimised form HTML generation

Generating XFORM `html` and `xml` is moved from being executed on every device to 
...
As of version 3.7.0, marking documents as no longer useful (purged) happens on the server. 
...
A warning is displayed, before starting initial replication, when the authenticated user would 
download more than the recommended maximum of 10.000 documents, in an attempt to reduce 
the incidence of misconfigured users. 
