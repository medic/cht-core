...
The changes in task storage ([see below](#tasks-data-vailable-for-analysis)) require configuration to be re-deployed using medic-conf 3.1.0 or greater to work properly. Medic-conf continues to work with cht-core releases prior to 3.8.0, so this re-deployment can happen at any time before upgrading.
