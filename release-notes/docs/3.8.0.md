...
- [Tasks data available for analysis](#tasks-data-available-for-analysis)
...
The changes in task storage ([see below](#tasks-data-vailable-for-analysis)) require configuration to be redeployed using medic-conf 3.1.0 or greater to work properly. It is recommended to redeploy the configuration before upgrading so your application will continue to work after the upgrade is complete.
...
## Tasks data available for analysis

Previously task data (their existence, whether and when they were resolved, etc) existed only on the user's device. With 3.8.0, this data is written to our data store, and is available for analysis. This is especially useful when combined with [couch2pg](https://github.com/medic/couch2pg), making the data available in postgres.

For more information, please read the [configuration guide](https://github.com/medic/medic-docs/blob/master/configuration/developing-community-health-applications.md#core-framework-380-and-later).
...
TODO measurable perf changes from task changes go here.
...
- [cht-core#5521](https://github.com/medic/cht-core/issues/5521): Make our audit-whitelist test a more courteous user of the API
