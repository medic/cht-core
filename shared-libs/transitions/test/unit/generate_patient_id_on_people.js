...
describe('generate_patient_id_on_people transition', () => {
  ...
  describe('filter', () => {
    ...
    it('should accept place contact types', () => {
      const doc = { type: DOC_TYPES.CONTACT, contact_type: 'place' };
      assert.equal(!!transition.filter({ doc }), true);
    });
  ...
  describe('onMatch', () => {
    it('should add patient_id to people', () => {
      const doc = { type: DOC_TYPES.CONTACT, contact_type: 'person' };
      sinon.stub(transitionUtils, 'getUniqueId').resolves('the_unique_id');
      return transition.onMatch({ doc }).then(result => {
        assert.equal(result, true);
        assert.deepEqual(doc, { type: DOC_TYPES.CONTACT, contact_type: 'person', patient_id: 'the_unique_id' });
      });
    });
    ...
    it('should add place_id to places', () => {
      ...
      return transition.onMatch({ doc }).then(result => {
        assert.equal(result, true);
        assert.deepEqual(doc, { type: DOC_TYPES.CONTACT, contact_type: 'place', place_id: 'the_unique_id' });
      });
    });
