...
/** @internal */
const devanagariToLatinCodePoint = (d: string): number => (d.codePointAt(0) ?? 0) - 0x0966 + 0x0030;
const normalizeDevanagariNumerals = (str: string): string => {
  return str.replace(/[०-९]/g, (d: string) => String.fromCodePoint(devanagariToLatinCodePoint(d)));
};
