...
describe('remote contact', () => {
  ...
  describe('v1', () => {
    ...
    describe('getPage', () => {
      ...
      it('returns a page of all contacts', async () => {
        const doc = [{ type: 'person' }, { type: 'person' }];
        const expectedResponse = { data: doc, cursor };
        getResourcesInner.resolves(expectedResponse);

        const result = await Contact.v1.getPage(remoteContext)(undefined, cursor, limit);

        expect(result).to.equal(expectedResponse);
        expect(getResourcesOuter.calledOnceWithExactly(remoteContext, 'api/v1/contact')).to.be.true;
        expect(getResourcesInner.calledOnceWithExactly({ limit: limit.toString(), cursor })).to.be.true;
      });
      ...
      it('omits cursor param when cursor is null', async () => {
        ...
        const result = await Contact.v1.getPage(remoteContext)(undefined, null, limit);
