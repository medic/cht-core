...
describe('contact', () => {
  ...
  describe('v1', () => {
    ...
    describe('getDatasource', () => {
      ...
      it('collectUuidsByPhone', async () => {
        const expectedIds = ['uuid-a', 'uuid-b'];
        const contactGetIds = sinon.stub().returns(fakeGenerator(expectedIds));
        dataContextBind.returns(contactGetIds);
        const phone = '+254712345678';
        const phoneQualifier = { phone };
        const byPhone = sinon.stub(Qualifier, 'byPhone').returns(phoneQualifier);
      ...
      it('collectUuidsByPhones', async () => {
        const expectedIds = ['uuid-a', 'uuid-b'];
        const contactGetIds = sinon.stub().returns(fakeGenerator(expectedIds));
        dataContextBind.returns(contactGetIds);
        const phones: [string, ...string[]] = ['+254712345678', '+254700000000'];
