  ...
  describe('taskDataFor', () => {
    ...
    it('unrecognized contact id yields empty', async () => {
      ...
      expect(db.query.args).to.deep.equal([
        ['shared-reports/reports_by_subject', { keys: ['abc'], include_docs: true, ...defaultQueryParams }],
        ['shared-reports/tasks_by_contact', { keys: ['requester-abc'], include_docs: true, ...defaultQueryParams }],
      ]);
    });
