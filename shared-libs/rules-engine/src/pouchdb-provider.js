...
const medicPouchProvider = db => {
  ...
  const self = {
    ...
    allTasks: prefix => {
      ...
      return docsOf(dbQuery('client-contacts/tasks_by_contact', options));
    },

    allTaskData: userSettingsDoc => {
      const userSettingsId = userSettingsDoc?._id;
      return Promise.all([
        docsOf(dbQuery('shared-contacts/contacts_by_type', { include_docs: true, reduce: false })),
        docsOf(dbQuery('client-reports/reports_by_subject', { include_docs: true })),
    ...
    contactsBySubjectId: async (subjectIds) => {
      ...
      const results = await db.query('shared-contacts/contacts_by_reference', { keys, include_docs: true });
    ...
    tasksByRelation: (contactIds, prefix) => {
      ...
      return docsOf(dbQuery( 'client-contacts/tasks_by_contact', { keys, include_docs: true }));
    },

    allTaskRowsByOwner: (contactIds) => {
      const keys = contactIds.map(contactId => (['owner', 'all', contactId]));
      return rowsOf(dbQuery( 'client-contacts/tasks_by_contact', { keys }));
    },
    ...
    allTaskRows: () => {
      ...
      return rowsOf(dbQuery( 'client-contacts/tasks_by_contact', options));
    },
    ...
    taskDataFor: async (contactIds, userSettingsDoc) => {
      ...
      const [reportDocs, taskDocs] = await Promise.all([
        docsOf(dbQuery('client-reports/reports_by_subject', { keys, include_docs: true })),
