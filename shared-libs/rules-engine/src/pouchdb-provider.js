...
const medicPouchProvider = db => {
  ...
  const self = {
    ...
    allTasks: prefix => {
      ...
      return docsOf(dbQuery('shared-reports/tasks_by_contact', options));
    },
    ...
    allTaskData: userSettingsDoc => {
      ...
      return Promise.all([
        ...
        docsOf(dbQuery('shared-reports/reports_by_subject', { include_docs: true })),
    ...
    tasksByRelation: (contactIds, prefix) => {
      ...
      return docsOf(dbQuery( 'shared-reports/tasks_by_contact', { keys, include_docs: true }));
    },

    allTaskRowsByOwner: (contactIds) => {
      const keys = contactIds.map(contactId => (['owner', 'all', contactId]));
      return rowsOf(dbQuery( 'shared-reports/tasks_by_contact', { keys }));
    },
    ...
    allTaskRows: () => {
      ...
      return rowsOf(dbQuery( 'shared-reports/tasks_by_contact', options));
    },
    ...
    taskDataFor: async (contactIds, userSettingsDoc) => {
      ...
      const [reportDocs, taskDocs] = await Promise.all([
        docsOf(dbQuery('shared-reports/reports_by_subject', { keys, include_docs: true })),
