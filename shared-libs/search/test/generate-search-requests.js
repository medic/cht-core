...
describe('GenerateSearchRequests service', () => {
  ...
  describe('validity filter', () => {
    ...
    it('true', () => {
      ...
      chai.expect(result.length).to.equal(1);
      chai.expect(result[0].view).to.equal('webapp-reports/reports_by_validity');
      chai.expect(result[0].params).to.deep.equal({
        key: [ true ]
      });
    });
  ...
  describe('verification filter', () => {
    ...
    it('queries', () => {
      ...
      verifiedValues.forEach((value) => {
        ...
        chai.expect(result.length).to.equal(1);
        chai.expect(result[0].view).to.equal('webapp-reports/reports_by_verification');
        chai.expect(result[0].params).to.deep.equal({
          keys: value.map((v) => [v])
        });
      });
  ...
  it('creates requests for reports with places filter', () => {
    ...
    const result = service('reports', filters);
    chai.expect(result.length).to.equal(1);
    chai.expect(result[0].view).to.equal('webapp-reports/reports_by_place');
    chai.expect(result[0].params).to.deep.equal({
      keys: [ [ 'a' ], [ 'b' ], [ 'c' ] ]
    });
  });
  ...
  it('creates requests for reports with subjectIds filter', () => {
    ...
    const result = service('reports', filters);
    chai.expect(result.length).to.equal(1);
    chai.expect(result[0].view).to.equal('shared-reports/reports_by_subject');
    chai.expect(result[0].params).to.deep.equal({
      keys: [ 'a', 'b', 'c' ]
    });
  });
