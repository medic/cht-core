...
const validityRequest = (filters) => {
  return getRequestForBooleanKey('client-reports/reports_by_validity', filters.valid);
};

const verificationRequest = (filters) => {
  return getRequestWithMappedKeys('client-reports/reports_by_verification', filters.verified, getKeysArray);
};

const placeRequest = (filters) => {
  return getRequestForMultidropdown('client-reports/reports_by_place', filters.facilities, getKeysArray);
};
...
const subjectRequest = (filters) => {
  ...
  return getRequestWithMappedKeys('client-reports/reports_by_subject', subjectIds);
};
...
const sortByLastVisitedDate = () => {
  return {
    view: 'client-contacts/contacts_by_last_visited',
    map: (row) => {
      row.id = row.key;
