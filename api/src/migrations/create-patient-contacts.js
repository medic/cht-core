...
var batchCreatePatientContacts = function(batch, settings, callback) {
  ...
  registrationIdsWithNoPatientContacts(batch, settings, function(err, registrationsToConsider) {
    if (err) {
      return callback(err);
    }

    process.stdout.write(registrationsToConsider.length + ' do not have a contact. ');
