...
var DDOC = {
  views: {
    audit_records_by_doc: {
      map:
        "function (doc) {if (doc.type === 'audit_record') {emit([doc.record_id], 1);}}",
    },
  },
};
var BATCH_SIZE = 100;
...
var batchMoveAuditDocs = function(callback) {
  db.medic.view(
    'medic-client',
    'doc_by_type',
    { key: ['audit_record'], limit: BATCH_SIZE },
    function(err, doclist) {
      ...
      var auditDocIds = doclist.rows.map(function(row) {
        return row.id;
      });

      async.parallel(
        [
          _.partial(db.db.replicate, db.settings.db, db.settings.auditDb, {
            doc_ids: auditDocIds,
          }),
