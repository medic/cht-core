...
const validateContact = (id, placeID) => {
  return db.medic.get(id)
    .then(doc => {
      if (doc.type !== 'person') {
        return Promise.reject(error400('Wrong type, contact is not a person.', { translationKey: 'contact.type.wrong'}));
      }
      if (!module.exports._hasParent(doc, placeID)) {
        return Promise.reject(error400('Contact is not within place.', {translationKey : 'configuration.user.place.contact' }));
      }
      return doc;
    });
};
...
const validateNewUsernameForDb = (username, database) => {
  return database.get(createID(username))
    ...
    .then(user => {
      if (user) {
        return Promise.reject(error400({ message: 'Username "'+ username +'" already taken.', translationKey: 'username.taken', field: 'username', parameter: username }));
      }
    });
};
...
const validateNewUsername = username => {
  if (!USERNAME_WHITELIST.test(username)) {
    return Promise.reject(error400('Invalid user name. Valid characters are lower case letters, numbers, underscore (_), and hyphen (-).', {key: 'username.invalid' }));
  }
  return Promise.all([
...
const validatePassword = password => {
  if (password.length < PASSWORD_MINIMUM_LENGTH) {
    return error400(`The password must be at least ${PASSWORD_MINIMUM_LENGTH} characters long.`, {translationKey: 'password.length.minimum', field: 'minimum', parameter: PASSWORD_MINIMUM_LENGTH });
  }
...
module.exports = {
  ...
  createUser: data => {
    ...
    if (missing.length > 0) {
      return Promise.reject(error400('Missing required fields: ' + missing.join(', '), {translationKey: 'fields.required'}));
    }
  ...
  updateUser: (username, data, fullAccess) => {
    ...
    if (!_.isNull(data.place) &&
      ...
      return Promise.reject(error400('One of the following fields are required: ' + props.join(', '), {translationKey: 'fields.one.required '}));
    }
    if (data.password) {
      const passwordError = validatePassword(data.password);
      if (passwordError) {
        ...
        return Promise.resolve()
          .then(() => {
            if (data.place) {
              ...
              if (settings.roles && isOffline(settings.roles)) {
                return Promise.reject(error400('Place field is required for offline users', {translationKey: 'field is required', field: 'field', parameter: 'Place'}));
              }
          ...
          .then(() => {
            if (data.contact) {
              ...
              if (settings.roles && isOffline(settings.roles)) {
                return Promise.reject(error400('Contact field is required for offline users', {translationKey: 'field is required', field: 'field', parameter: 'Contact'}));
              }
