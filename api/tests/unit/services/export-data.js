...
exports['get handles empty db'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: []
  });
  service.get({ type: 'messages' }, (err, results) => {
...
exports['get formats responses'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'messages', tz: '0' }, (err, results) => {
...
exports['get includes tasks and scheduled tasks'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
...
exports['get formats incoming messages'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
...
exports['get exports messages in xml'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'messages', tz: '0', format: 'xml' }, function(err, streamFn) {
...
exports['get exports reports in xml with each type on a separate tab'] = test => {
  test.expect(3);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  var expected =  '<?xml version="1.0" encoding="UTF-8"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:html="http://www.w3.org/TR/REC-html140" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><?mso-application progid="Excel.Sheet"?>' +
                    ...
                    '</Table></Worksheet>' +
                  '</Workbook>';
  service.get({ type: 'forms', tz: '0', format: 'xml', columns: '[ "_id" ]' }, function(err, streamFn) {
    readStream(streamFn, function(results) {
      test.equals(results, expected);
      test.equals(getView.callCount, 1);
...
exports['if form definition not found then cannot add specific columns'] = test => {
  test.expect(3);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
...
exports['get uses locale param'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'messages', tz: '0', locale: 'fr' }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
    test.done();
  });
...
exports['get uses tz param'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'messages', tz: '-120' }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
    test.done();
  });
...
exports['get uses skip_header_row param'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'messages', tz: '0', skip_header_row: true }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
    test.done();
  });
...
exports['get uses columns param'] = test => {
  test.expect(2);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'messages', tz: '0', columns: '["reported_date","from","contact.parent.name"]' }, (err, results) => {
...
exports['get uses filter state params'] = test => {
  test.expect(2);
  ...
  service.get({ type: 'messages', columns: '["_id"]', tz: '0', filter_state: 'pending', filter_state_from: '-20', filter_state_to: '-10' }, (err, results) => {
...
exports['get reports formats responses'] = test => {
  test.expect(6);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'forms', tz: '0' }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
...
exports['get reports filters by form'] = test => {
  test.expect(6);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'forms', tz: '0', form: 'P' }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
    test.equals(getView.firstCall.args[1], 'data_records');
...
exports['get reports works for enketo reports'] = test => {
  test.expect(6);
  var getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'forms', tz: '0', form: 'assessment', columns: '[ "_id", "contact._id", "fields.patient_id", "reported_date" ]' }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
    test.equals(getView.firstCall.args[1], 'data_records');
...
exports['get reports hydrates contacts'] = test => {
  ...
  const getView = sinon.stub(db.medic, 'view').callsArgWith(3, null, {
    rows: [
      { doc: {
        _id: 'B87FEE75-D435-A648-BDEA-0A1B61021AA3',
        _rev: '3-e361f7bce1a97799b3265336a2e68f11',
...
exports['get reports with query calls fti'] = test => {
  test.expect(9);
  ...
  service.get({ type: 'forms', query: 'form:P', tz: '0' }, (err, results) => {
...
exports['get audit log'] = test => {
  ...
  service.get({ type: 'audit', tz: '0' }, (err, results) => {
    test.equals(results, expected);
...
exports['get audit log handles special characters'] = test => {
  test.expect(2);
  var list = sinon.stub(db.audit, 'list').callsArgWith(1, null, {
    rows: [
      { doc: {
  ...
  service.get({ type: 'audit', tz: '0', format: 'xml' }, function(err, streamFn) {
    readStream(streamFn, function(results) {
      test.equals(results, expected);
      test.equals(list.callCount, 1);
      test.done();
...
exports['get feedback'] = test => {
  ...
  service.get({ type: 'feedback', tz: '0' }, (err, results) => {
    test.equals(results, expected);
    test.equals(getView.callCount, 1);
...
exports['get contacts'] = test => {
  ...
  service.get({ type: 'contacts', query: 'district:2', format: 'json', tz: '0' }, (err, results) => {
