...
describe('Initial Replication service', () => {
  ...
  describe('pushDocs', () => {
    it('should return empty array for empty docs', async () => {
      const result = await replication.pushDocs(userCtx, []);
      expect(result).to.deep.equal([]);
    });

    it('should filter docs by authorization and store allowed ones', async () => {
      const docs = [
        { _id: 'doc1', _rev: '1-abc', type: 'report' },
        { _id: 'doc2', _rev: '1-def', type: 'contact' },
        { _id: 'doc3', _rev: '1-ghi', type: 'report' },
      ];
      sinon.stub(authorization, 'getAuthorizationContext').resolves(authContext);
      sinon.stub(authorization, 'getViewResults').returns({});
      sinon.stub(authorization, 'filterAllowedDocs')
        .callsFake((ctx, docObjs) => docObjs.filter(d => d.doc._id !== 'doc2'));
      // new_edits: false returns empty array on success
      sinon.stub(db.medic, 'bulkDocs').resolves([]);

      const result = await replication.pushDocs(userCtx, docs);

      expect(authorization.getAuthorizationContext.args).to.deep.equal([[userCtx]]);
      expect(db.medic.bulkDocs.callCount).to.equal(1);
      expect(db.medic.bulkDocs.args[0][0]).to.deep.equal([docs[0], docs[2]]);
      expect(db.medic.bulkDocs.args[0][1]).to.deep.equal({ new_edits: false });
      expect(result).to.deep.equal([
        { ok: true, id: 'doc1', rev: '1-abc' },
        { id: 'doc2', error: 'forbidden' },
        { ok: true, id: 'doc3', rev: '1-ghi' },
      ]);
    });

    it('should return all forbidden when no docs are allowed', async () => {
      const docs = [{ _id: 'doc1', _rev: '1-abc', type: 'report' }];
      sinon.stub(authorization, 'getAuthorizationContext').resolves(authContext);
      sinon.stub(authorization, 'getViewResults').returns({});
      ...
      expect(result).to.deep.equal([{ id: 'doc1', error: 'forbidden' }]);
    });

    it('should throw authorization errors', async () => {
      sinon.stub(authorization, 'getAuthorizationContext').rejects(new Error('auth failed'));

      await expect(replication.pushDocs(userCtx, [{ _id: 'doc1' }])).to.be.rejectedWith(Error, 'auth failed');
    });

    it('should throw bulkDocs errors', async () => {
      sinon.stub(authorization, 'getAuthorizationContext').resolves(authContext);
      sinon.stub(authorization, 'getViewResults').returns({});
      sinon.stub(authorization, 'filterAllowedDocs').callsFake((ctx, docObjs) => docObjs);
      sinon.stub(db.medic, 'bulkDocs').rejects(new Error('db error'));

      await expect(replication.pushDocs(userCtx, [{ _id: 'doc1' }])).to.be.rejectedWith(Error, 'db error');
    });
  });
});
