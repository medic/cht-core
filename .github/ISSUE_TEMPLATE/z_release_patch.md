...
- [ ] Upgrade the [demo](https://demo-cht.dev.medicmobile.org/) instance to the newly released version by running the [Deploy Demo Instance](https://github.com/medic/cht-core/actions/workflows/deploy-demo.yml) workflow. Click "Run workflow", enter the release tag (e.g. `<major>.<minor>.<patch>`), and confirm. The workflow will automatically stage the upgrade, perform the helm deploy, verify the version, and upload the demo configuration.
