...
module.exports = {
  ...
  getNextTimes: function(doc, now) {
    ...
    if (due && now) {
      _.each(['minutes', 'hours', 'days', 'weeks', 'months', 'years'], function(
        unit
      ) {
    ...
    _.each(schedule.messages, function(msg) {
      ...
      if (offset) {
        ...
        try {
          ...
          const state = messages.isOutgoingAllowed(doc.from)
            ? 'scheduled'
            : 'denied';
        ...
        logger.error(
          '%s cannot be parsed as a valid offset. Skipping this msg of %s schedule.',
          msg.offset,
          schedule.name
        );
      }
