...
describe('bulk-operations delete handler', () => {
  ...
  it('copies the docs, then deletes them from medic', async () => {
  ...
  it('skips a doc that is already deleted or purged and still deletes the others', async () => {
    db.medic.allDocs.resolves({ rows: [
      ...
      { key: 'deleted', id: 'deleted', value: { rev: '2-x', deleted: true } },
      { doc: report },
    ] });
    db.medic.bulkDocs.resolves([ { ok: true, id: 'b' } ]);

    const failed = await deleteDocs([ { id: 'gone' }, { id: 'deleted' }, { id: 'b' } ], 'action-1');
  ...
  it('does not copy or delete anything when every doc is already gone', async () => {
    db.medic.allDocs.resolves({ rows: [
      { key: 'gone', error: 'not_found' },
      { key: 'deleted', id: 'deleted', value: { rev: '2-x', deleted: true } },
    ] });

    const failed = await deleteDocs([ { id: 'gone' }, { id: 'deleted' } ], 'action-1');
