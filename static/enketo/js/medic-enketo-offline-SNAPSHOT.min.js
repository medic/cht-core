...
b);b=(new d(function(b){var d=a.ownerDocument;return d.evaluate.call(d,b,a,f,XPathResult.STRING_TYPE,k)},g)).evaluate(b);console.log("    -> "+b);return b};window.JsXPathException=window.JsXPathExpression=window.JsXPathNSResolver=window.JsXPathResult=window.JsXPathNamespace=!0};a.prototype.getInstanceID=function(){return this.node("/*/meta/instanceID").getVal()[0]};a.prototype.getInstanceName=function(){return this.node("/*/meta/instanceName").getVal()[0]};a.prototype.cloneRepeat=function(b,a){var d,
f,h,g=this.templates[b]||this.node(b,0).get();f=g.prop("nodeName");d=this.node(b,a).get();g[0]&&1===d.length&&0===d.nextAll(f).length?(d=g.clone().insertAfter(d),h=[g.prop("nodeName")],g.find("*").each(function(){h.push(k(this).prop("nodeName"))}),this.$.trigger("dataupdate",{nodes:h,repeatPath:b,repeatIndex:this.node(b,a).determineIndex(d)})):0===d.nextAll(f).length&&console.error("Could not find template node and/or node to insert the clone after")};a.prototype.extractTemplates=function(){var b=
