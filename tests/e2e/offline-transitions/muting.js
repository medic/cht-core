...
describe('Muting', () => {
  ...
  const settings = {
    transitions: { muting: { client: true } },
    muting: {
  ...
  describe('for an offline user', () => {
    ...
    it('should mute and unmute a person while "offline", with processing in between', async () => {
      ...
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [{ muted: true, date: updatedPatient1.muted, report_id: mutingReport._id }],
      });
      ...
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: true, date: updatedPatient1.muted },
        client: [{ muted: true, date: clientMutingDate, report_id: mutingReport._id }],
      });
      ...
      const unmutingDate = updatedPatient1.muting_history.client[1].date;
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: true, date: serverMutedDate },
        client: [
      ...
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: false, date: updatedPatient1.muting_history.server.date /* can't guess this date */ },
        client: [
          { muted: true, date: clientMutingDate, report_id: mutingReport._id },
    ...
    it('should mute and unmute a clinic while "offline", with processing in between', async () => {
      ...
      expect(updatedClinic1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [{ muted: true, date: updatedClinic1.muted, report_id: mutingReport._id }],
      });
      ...
      expect(updatedPatient3.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [{ muted: true, date: updatedPatient3.muted, report_id: mutingReport._id }],
      });
      ...
      expect(updatedPatient3.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: true, date: updatedPatient3.muted },
        client: [{ muted: true, date: clientMutingDate, report_id: mutingReport._id }],
      });
      ...
      expect(updatedClinic1.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: true, date: updatedClinic1.muted },
        client: [{ muted: true, date: clientMutingDate, report_id: mutingReport._id }],
      });
      ...
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: true, date: patient1ServerMutingDate },
        client: [
          { muted: true, date: clientMutingDate, report_id: mutingReport._id },
          { muted: false, date: unmutingDate, report_id: unmutingReport._id },
        ],
      ...
      expect(updatedClinic1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: true, date: clinic1ServerMutingDate },
        client: [
          { muted: true, date: clientMutingDate, report_id: mutingReport._id },
          { muted: false, date: unmutingDate, report_id: unmutingReport._id },
        ],
      ...
      expect(updatedPatient1.muted).to.be.undefined;
      let serverUnmutingDate = updatedPatient1.muting_history.server.date;
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: false, date: serverUnmutingDate },
        client: [
          { muted: true, date: clientMutingDate, report_id: mutingReport._id },
          { muted: false, date: unmutingDate, report_id: unmutingReport._id },
        ],
      ...
      updatedClinic1 = await utils.getDoc(clinic1._id);
      serverUnmutingDate = updatedClinic1.muting_history.server.date;
      expect(updatedClinic1.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: false, date: serverUnmutingDate },
        client: [
    ...
    it('should mute and unmute a clinic while "offline", without processing in between', async () => {
      ...
      updatedPatient1 = await utils.getDoc(patient1._id);
      expect(updatedPatient1.muted).to.be.undefined;
      const patient1UnmutingDate = updatedPatient1.muting_history.client[1].date;
      expect(updatedPatient1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [
      ...
      updatedClinic1 = await utils.getDoc(clinic1._id);
      expect(updatedClinic1.muted).to.be.undefined;
      const clinic1UnmutingDate = updatedClinic1.muting_history.client[1].date;
      expect(updatedClinic1.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [
      ...
      expect(updatedClinic1.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: false, date: updatedClinic1.muting_history.server.date /* can't guess this date */},
        client: [
          { muted: true, date: clientMutingDate, report_id: mutingReport._id },
          { muted: false, date: clinic1UnmutingDate, report_id: unmutingReport._id },
        ],
    ...
    it('should mute a clinic and unmute a patient while "offline", without processing in between', async () => {
      ...
      const patient3unmutingDate = updatedPatient3.muting_history.client[1].date;
      expect(updatedPatient3.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [
      ...
      expect(updatedPatient3.muting_history).to.deep.equal({
        last_update: 'server',
        server: { muted: false, date: updatedPatient3.muting_history.server.date /* can't guess this date */ },
        client: [
          { muted: true, date: clientMutingDate, report_id: mutingReport._id },
          { muted: false, date: patient3unmutingDate, report_id: unmutingReport._id },
        ],
    ...
    it('should handle offline multiple muting/unmuting events gracefully', async () => {
      ...
      expect(updatePatient1.muting_history.last_update).to.equal('client');
      expect(updatePatient1.muting_history.server).to.deep.equal({ muted: false });
      expect(updatePatient1.muting_history.client.length).to.equal(7);

      let updatedClinic = await utils.getDoc(clinic1._id);
      expect(updatedClinic.muted).to.be.undefined;
      expect(updatedClinic.muting_history.last_update).to.equal('client');
      expect(updatedClinic.muting_history.server).to.deep.equal({ muted: false });
      expect(updatedClinic.muting_history.client.length).to.equal(4);

      await utils.startSentinel();
      const reportIds = updatePatient1.muting_history.client.map(event => event.report_id);
      await sentinelUtils.waitForSentinel(reportIds);

      updatePatient1 = await utils.getDoc(patient1._id);
      expect(updatePatient1.muted).to.be.ok;
      expect(updatePatient1.muting_history.last_update).to.equal('server');
      expect(updatePatient1.muting_history.server).to.deep.equal({ muted: true, date: updatePatient1.muted });
      expect(updatePatient1.muting_history.client.length).to.equal(7);

      updatedClinic = await utils.getDoc(clinic1._id);
      expect(updatedClinic.muted).to.be.undefined;
      expect(updatedClinic.muting_history.last_update).to.equal('server');
      expect(updatedClinic.muting_history.server.muted).to.equal(false);
      expect(updatedClinic.muting_history.client.length).to.equal(4);
    });
    ...
    it('should save validation errors on docs', async () => {
      ...
      expect(report.client_transitions).to.be.undefined;
    ...
    it('should work with composite forms', async () => {
      ...
      expect(newClinic.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [{ muted: true, date: newClinic.muted, report_id: mutingReport._id }],
      });
      ...
      expect(newPerson.muting_history).to.deep.equal({
        last_update: 'client',
        server: { muted: false },
        client: [{ muted: true, date: newPerson.muted, report_id: mutingReport._id }],
      });

      expect(mutingReport.client_transitions.muting).to.equal(true);
      expect(mainReport.client_transitions).to.be.undefined;
      ...
      expect(updatedNewClinic.muting_history.last_update).to.equal('server');
      expect(updatedNewClinic.muting_history.server.muted).to.equal(true);

      const updatedNewPerson = await utils.getDoc(newPerson._id);
      expect(updatedNewPerson.muted).to.be.ok;
      expect(updatedNewPerson.muting_history.last_update).to.equal('server');
      expect(updatedNewPerson.muting_history.server.muted).to.equal(true);
