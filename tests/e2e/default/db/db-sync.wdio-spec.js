...
describe('db-sync', () => {
  ...
  it('should filter ddocs', async () => {
    ...
    const serverRevs = await getServerRevs([DDOC_IDS.MEDIC_CLIENT]);
    await chtDbUtils.updateDoc(DDOC_IDS.MEDIC_CLIENT, { something: 'random' });
    // updating the ddoc will throw the "upgrade" popup, ignore it!
    await commonElements.closeReloadModal();
    await commonElements.sync();

    const updatedServerRevs = await getServerRevs([DDOC_IDS.MEDIC_CLIENT]);
    expect(serverRevs).to.deep.equal(updatedServerRevs);
