...
describe('Reports tab breadcrumbs', () => {
  ...
  const offlineUserContact = personFactory.build(  { name: 'OfflineUser', parent: healthCenter1 });

  const onlineUserContact = personFactory.build(  { name: 'OnlineUser', parent: districtHospital });
  ...
  const patient = personFactory.build({ _id: 'patient1', parent: clinic });
