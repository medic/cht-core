...
const aboutPage = require('../../page-objects/common/about.wdio.page');
...
describe('About page' , async () => {
  ...
  it('should display partners', async () => {
    ...
    const image1 = await aboutPage.getPartnerImage('partner');
    chai.expect(image1).to.equal(`data:image/png;base64,${partnersDoc._attachments.image1.data}`);
    const image2 = await aboutPage.getPartnerImage('medic');
    chai.expect(image2).to.equal(`data:image/png;base64,${partnersDoc._attachments.image2.data}`);
  });
});
