describe('Contact service', function() {
  ...
  it('returns zero when no messages', function() {
  ...
  it('returns all persons with phones', function() {
  ...
  it('returns "everyone at" when health_center', function() {
    ...
    return service().then(function(actual) {
      ...
      chai.expect(Facility.args[0][0].district).to.equal('abc');
      chai.expect(Facility.args[0][0].types).to.deep.equal([ 'person', 'health_center' ]);
      chai.expect(Facility.args[0][0].targetScope).to.equal('root');
      chai.expect(Facility.callCount).to.equal(1);
    });
  });
  ...
  it('returns err from user district', function(done) {
    UserDistrict.returns(KarmaUtils.mockPromise('boom'));
    service()
      .then(function() {
        done(new Error('expected error to be thrown'));
      })
