...
const feedBackDocs = async (testName = 'allLogs') => {
  ...
  if (feedBackDocs && feedBackDocs.flat().length > 0) {
    fs.writeFileSync(`./tests/logs/feedbackDocs-${testName}.txt`, JSON.stringify(feedBackDocs));
    return false;
  }
};


const feedBackDocsScript = async () => {
  ...
  return Promise.all(metaDbList.map(async (db) => {
