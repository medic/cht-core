const chai = require('chai'),
      ...
      sinon = require('sinon'),
      calendarInterval = require('../../../../static/js/modules/calendar-interval');
...
describe('CalendarInterval module', () => {
  describe('getCurrent', () => {
    it('returns 1st of current month when month start is not set or incorrect', () => {
      ...
      chai.expect(calendarInterval.getCurrent()).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(0)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(-1)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(false)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(100)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent('something')).to.deep.equal({ start: expectedStart, end: expectedEnd });
      clock.restore();
      ...
      chai.expect(calendarInterval.getCurrent()).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(0)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(-10)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(false)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent({})).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent(undefined)).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent([])).to.deep.equal({ start: expectedStart, end: expectedEnd });
      chai.expect(calendarInterval.getCurrent('something')).to.deep.equal({ start: expectedStart, end: expectedEnd });
    });
    ...
    it('returns 1st of current month when month start is 1', () => {
      clock = sinon.useFakeTimers(moment('2018-01-20 21:10:01').valueOf());
      chai.expect(calendarInterval.getCurrent(1)).to.deep.equal({
    ...
    it('returns n-th of the current month when month start is n <= today', () => {
      clock = sinon.useFakeTimers(moment('2018-01-20 21:10:01').valueOf());
      chai.expect(calendarInterval.getCurrent(10)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(6)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(18)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(20)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(9)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(30)).to.deep.equal({
    ...
    it('returns n-th of the previous month when month start is n > today', () => {
      ...
      chai.expect(calendarInterval.getCurrent(10)).deep.to.equal({
        start: moment('2018-01-10 00:00:00').valueOf(),
        end: moment('2018-02-09 23:59:59:999').valueOf()
      });
      clock.restore();
      ...
      clock = sinon.useFakeTimers(moment('2018-03-18 11:10:01').valueOf());
      chai.expect(calendarInterval.getCurrent(25)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(29)).to.deep.equal({
    ...
    it('returns first day of current month when start date exceeds previous month day number', () => {
      ...
      chai.expect(calendarInterval.getCurrent(31)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(28)).to.deep.equal({
      ...
      chai.expect(calendarInterval.getCurrent(27)).to.deep.equal({
