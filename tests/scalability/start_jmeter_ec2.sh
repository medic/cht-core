...
# base64 encode our script and set as value for UserDAta in launch-specification
 jq --arg arn ${{ secrets.SCALABILITY_ARN }} '.UserData = "'$(base64 run_suite.sh -w 0)'" | .IamInstanceProfile.Arn = $arn' launch-specification.json >> launch-specification-jmeter.json
