...
const bracSurvey = () => {
    return new Factory()
        ...
        .attr('fields', ['form', 'contact', 'patient'], (form, contact, patient) => {
            if (form === 'household_survey') { return householdSurveyFactory.build({}, { contact: contact }); }
            if (form === 'family_survey') { return familydSurveyFactory.build({}, { contact: contact }); }
            if (form === 'pregnancy') { return pregnancyFactory.build({}, { patient: patient, contact: contact }); }
...
const generateBracSurvey = function (form, parentPlace, place, patient) {
    const contactParent = {
