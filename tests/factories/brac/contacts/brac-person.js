...
const bracPerson = () => {
    return new Factory()
        ...
        .attr('sex', ['subtype'], (subtype) => {
            if (subtype === 'member_eligible_woman') { return 'female'; }
            if (subtype === 'family_member') { return 'male'; }
            else { return Faker.faker.name.gender(true).toLowerCase(); }
        })
        ...
        .attr('age_years', ['subtype', 'date_of_birth_method'], (subtype, dateOfBirthMethod) => {
            if (dateOfBirthMethod !== 'approx') { return null; }
            if (subtype === 'member_child') { return Faker.faker.datatype.number({ min: 0, max: 5 }); }
            if (subtype === 'member_eligible_woman' || subtype === 'manager' || subtype === 'chw') { return Faker.faker.datatype.number({ min: 12, max: 52 }); }
            else { return Faker.faker.datatype.number({ min: 6, max: 80 }); }
        })
        .attr('age_months', ['date_of_birth_method'], (dateOfBirthMethod) => {
            if (dateOfBirthMethod !== 'approx') { return null; }
            return Faker.faker.datatype.number({ min: 0, max: 12 });
        })
        .attr('date_of_birth', ['subtype', 'date_of_birth_method', 'age_years', 'age_months'], (subtype, dateOfBirthMethod, ageYears, ageMonths) => {
            if (dateOfBirthMethod === 'approx') {
                let amount = ageYears * 12 + ageMonths;
        ...
        .attr('phone', ['subtype'], (subtype) => {
            if (subtype === 'member_child') { return null; }
            else { return Faker.faker.phone.phoneNumber('+256(###) ### ### ###'); }
        })
        ...
        .attr('family_member_type', ['subtype'], (subtype) => {
            if (subtype !== 'member_child' && subtype !== 'member_eligible_woman') { return null; }
            else { return subtype; }
        })
        .attr('other_name', ['subtype'], (subtype) => {
            if (subtype !== 'chw') { return null; }
            else { return Faker.faker.name.firstName(); }
        })
        ...
        .attr('chp_profile', ['subtype', 'date_of_birth', 'sex', 'phone', 'phone_alternate'], (subtype, dateOfBirth, sex, phone, phoneAlternate) => {
            if (subtype !== 'chw') { return null; }
            else { return chpProfile.build({}, { dob: dateOfBirth, sex: sex, phone_number: phone, alternate_number: phoneAlternate }); }
        })
        .attr('pregnant_at_registration', ['subtype'], (subtype) => {
            if (subtype !== 'member_eligible_woman') { return null; }
        ...
        .attr('group_other_woman_pregnancy', ['pregnant_at_registration', 'subtype'], (pregnantAtRegistration, subtype) => {
            if (subtype !== 'member_eligible_woman') { return null; }
            else { return groupOtherWomanPregnancy.build({ other_woman_pregnant: pregnantAtRegistration }); }
        })
        .attr('c_name', ['subtype', 'name'], (subtype, name) => {
            if (subtype === 'chw' || subtype === 'manager') { return null; }
            else { return name; }
        })
        .attr('dob_method', ['subtype', 'date_of_birth_method'], (subtype, dateofBirthMethod) => {
            if (subtype === 'chw' || subtype === 'manager') { return null; }
            else { return dateofBirthMethod; }
        })
        .attr('ephemeral_months', ['subtype', 'date_of_birth'], (subtype, dateofBirth) => {
            if (subtype === 'chw' || subtype === 'manager') { return null; }
            else { return new Date(dateofBirth).getMonth() + 1; }
        })
