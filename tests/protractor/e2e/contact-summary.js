...
describe('Contact summary info', () => {
  ...
  it('contact summary', () => {
    ...
    expect(helper.getTextFromElement('.content-pane .meta > .card .col-sm-3:nth-child(1) label')).toBe('test.pid');
    expect(helper.getTextFromElement('.content-pane .meta > .card .col-sm-3:nth-child(1) p')).toBe(CAROL.patient_id);
    expect(helper.getTextFromElement('.content-pane .meta > .card .col-sm-3:nth-child(2) label')).toBe('test.sex');
    expect(helper.getTextFromElement('.content-pane .meta > .card .col-sm-3:nth-child(2) p')).toBe(CAROL.sex);

    // assert that the pregnancy card exists and has the right fields.
    expect(helper.getTextFromElement('.content-pane .meta > div > .card .action-header h3')).toBe('test.pregnancy');
    expect(helper.getTextFromElement('.content-pane .meta > div > .card .row label')).toBe('test.visits');
    expect(helper.getTextFromElement('.content-pane .meta > div > .card .row p')).toBe('1');
  });
});
