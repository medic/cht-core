...
exports['indexes doc name and type'] = function(test) {
  // given
  ...
  // then
  utils.assertIncludesPair(test, emitted, ['person', 'test']);
  utils.assertIncludesPair(test, emitted, ['person', 'clinic']);
  utils.assertIncludesPair(test, emitted, ['person', 'contact']);
  test.done();
};
...
exports['indexes non-ascii doc name'] = function(test) {
  ...
  // then
  utils.assertIncludesPair(test, emitted, ['person', 'बुद्ध']);
  utils.assertIncludesPair(test, emitted, ['person', 'élève']);
  test.done();
};
...
exports['does not index words of less than 3 chars'] = function(test) {
  ...
  // then
  utils.assertDoesNotIncludePair(test, emitted, ['person', 'of']);
  test.done();
};
