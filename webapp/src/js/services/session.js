...
(function() {
  ...
  inboxServices.factory('Session', function($http, $log, $window, ipCookie, Location) {
    'ngInject';
    ...
    var checkCurrentSession = function(callback) {
      ...
      $http
        ...
        .then(function(response) {
          var name = response.data && response.data.userCtx && response.data.userCtx.name;
          if (name !== userCtx.name) {
            // connected to the internet but server session is different
            return logout();
          }
          if (
            _.difference(userCtx.roles, response.data.userCtx.roles).length ||
    ...
    var isAdmin = function(userCtx) {
      ...
      return hasRole(userCtx, '_admin') || hasRole(userCtx, 'national_admin'); // deprecated: kept for backwards compatibility: #4525
    };
    ...
    return {
      ...
      isOnlineOnly: function(userCtx) {
        userCtx = userCtx || getUserCtx();
        return isAdmin(userCtx) || hasRole(userCtx, ONLINE_ROLE);
      },
    };
  });
})();
