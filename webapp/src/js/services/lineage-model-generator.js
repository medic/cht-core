 ...
 * }
 */
angular.module('inboxServices').factory('LineageModelGenerator', function($q, DB) {
  'ngInject';
  'use strict';
  var lineage = lineageFactory($q, DB());

  var get = function(id) {
    return lineage.fetchLineageById(id).then(function(docs) {
      if (!docs.length) {
  ...
  var hydrate = function(docs) {
    return lineage.fetchContacts(docs).then(function(contacts) {
  ...
  return {
    ...
    reportSubjects: function(ids) {
      return lineage.fetchLineageByIds(ids).then(function(docsList) {
