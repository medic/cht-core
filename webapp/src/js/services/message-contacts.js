...
angular
  ...
  .factory('MessageContacts', function(AddReadStatus, DB, GetDataRecords, HydrateMessages) {
    'use strict';
    'ngInject';
        ...
        .then(function(response) {
          ...
          if (params.reduce === undefined || params.reduce === true) {
            ...
            return GetDataRecords(ids, { include_docs: true }).then(function(docs) {
              _.each(response.rows, function(row, idx) {
                row.doc = docs[idx];
              });
    ...
    return {
      list: function() {
        return getMessages(listParams()).then(AddReadStatus.messages);
      },
      conversation: function(id, skip) {
        return getMessages(conversationParams(id, skip || 0)).then(AddReadStatus.messages);
      },
    };
  });
