...
angular.module('inboxServices').service('ContactSave', function($q, DB, EnketoTranslation, ExtractLineage) {
  'use strict';
  'ngInject';
  ...
  function prepareSubmittedDocsForSave(schema, original, submitted) {
    ...
    return prepareAndAttachSiblingDocs(schema, submitted.doc, original, submitted.siblings).then(function(siblings) {
    ...
    _.each(schema.fields, function(fieldConf, fieldName) {
      ...
      if (customType) {
        ...
        if (!value) {
          ...
          var isChild =
            preparedSibling.parent === 'PARENT' || (!preparedSibling.parent && fieldConf.parent === 'PARENT');
