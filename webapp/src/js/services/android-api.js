 ...
 */
angular.module('inboxServices').factory('AndroidApi', function($log, $rootScope, $state, MRDT, Session, Simprints) {
  'use strict';
  'ngInject';
  ...
  var closeTopModal = function($modals) {
    ...
    if (!closeDropdownsIn($topModal)) {
      ...
      // button.
      $topModal.find('.btn.cancel:visible:not(:disabled),' + 'button.close:visible:not(:disabled)').click();
    }
  };
