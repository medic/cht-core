...
angular.module('inboxServices').factory('GetSubjectSummaries', function($q, DB, GetSummaries, LineageModelGenerator) {
  'use strict';
  'ngInject';
  ...
  var processContactIds = function(summaries) {
    ...
    return GetSummaries(ids).then(function(response) {
  ...
  var compactSubjectLineage = function(summaries) {
    return _.each(summaries, function(summary) {
      if (summary.subject && summary.subject.lineage) {
        summary.subject.lineage = _.compact(
          _.map(summary.subject.lineage, function(parent) {
  ...
  var processSubjectLineage = function(summaries) {
    ...
    return LineageModelGenerator.reportSubjects(subjectIds).then(function(response) {
