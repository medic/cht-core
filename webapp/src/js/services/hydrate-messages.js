...
angular.module('inboxServices').factory('HydrateMessages', function($q, DB, LineageModelGenerator) {
  'use strict';
  'ngInject';
  ...
  return function(rows) {
    ...
    return LineageModelGenerator.reportSubjects(contactIds).then(function(reports) {
      ...
      return rows.map(function(row) {
        return buildMessageModel(row.doc, row.key[0], row.value.date, row.report);
      });
    });
  };
});
