...
angular.module('inboxServices').service('ContactForm', function(ContactSchema, DB, EnketoTranslation) {
  'use strict';
  'ngInject';

  var withAvailableForms = DB()
    .query('medic-client/forms')
    .then(function(res) {
  ...
  var getFormFor = function(type, mode, extras) {
    return withAvailableForms.then(function(availableForms) {
      return (
        getFormById(availableForms, 'form:contact:' + type + ':' + mode) ||
