(function() {
  ...
  // Else throw error.
  inboxServices.factory('UserDistrict', function($q, DB, Session) {
    'ngInject';
    ...
    return function() {
      ...
      return getWithRemoteFallback(DB, 'org.couchdb.user:' + userCtx.name)
        ...
        .then(function(facilityId) {
          ...
          return getWithRemoteFallback(DB, facilityId).then(function() {
            return facilityId;
          });
        });
    };
  ...
  inboxServices.factory('UserSettings', function($q, DB, Session) {
    'ngInject';
    return function() {
      var userCtx = Session.userCtx();
      if (!userCtx) {
