...
describe('Form service', () => {
  ...
  describe('save', () => {
    ...
    it('populates the diverged-children accumulator on edit divergence, without showing a snackbar', () => {
      form.validate.resolves(true);
      ...
      // The live linked doc diverged on some_property_2 since the report was last saved.
      dbAllDocs.resolves({
        rows: [{
          id: 'child-1',
          doc: {
            _id: 'child-1', _rev: '9-child', type: 'thing_1',
            some_property_1: 'snap1', some_property_2: 'changed_elsewhere', reported_date: 500,
          },
        }],
      });
      dbBulkDocs.callsFake(docs => Promise.resolve(docs.map(doc => ({ ok: true, id: doc._id, rev: `2-${doc._id}` }))));

      const divergedChildren = [];
      return service.save('V', form, null, '6', divergedChildren).then(() => {
        expect(divergedChildren).to.deep.equal([{ id: 'child-1', type: 'thing_1' }]);
        // FormService forwards the accumulator but never shows a snackbar itself.
        expect(globalActions.setSnackbarContent.called).to.be.false;
      });
    });

    it('leaves the diverged-children accumulator empty when nothing changed elsewhere', () => {
      form.validate.resolves(true);
      form.getDataStr.returns(`
        <data>
          <name>Sally</name>
      ...
      // The live linked doc matches the last report snapshot - no divergence.
      dbAllDocs.resolves({
        rows: [{
          id: 'child-1',
          doc: {
            _id: 'child-1', _rev: '9-child', type: 'thing_1',
            some_property_1: 'snap1', some_property_2: 'snapshot_val', reported_date: 500,
          },
        }],
      });
      dbBulkDocs.callsFake(docs => Promise.resolve(docs.map(doc => ({ ok: true, id: doc._id, rev: `2-${doc._id}` }))));
