describe('AboutCtrl controller', function() {
  ...
  it('initializes data', () => {
    ...
    DB.allDocs.withArgs({ key: '_design/medic-client' }).resolves({ rows: [{ value: { rev: '4465-bbbb' }}] });
    DB.get.withArgs('medic-deploy-info').resolves({ deploy_info: { version: 4 } });
    createController();
  ...
  it('displays base version if exists', () => {
    DB.get.withArgs('medic-deploy-info').resolves({ deploy_info: { version: '4-beta', base_version: '4' } });
    createController();
  ...
  it('displays package version if doc not found', () => {
    DB.get.withArgs('medic-deploy-info').rejects();
  ...
  it('displays package version if doc does not have info', () => {
    DB.get.withArgs('medic-deploy-info').resolves({});
    scope.version = 'some version';
    createController();
