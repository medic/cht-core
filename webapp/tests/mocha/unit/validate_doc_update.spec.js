...
describe('validate doc update', () => {
  ...
  const clientValidateDocUpdate = function() {
    /*jshint -W061 */
    const fn = fs.readFileSync('./webapp/src/ddocs/medic-client/validate_doc_update.js');
    eval('(' + fn + ')').apply(null, arguments);
  };

  const serverValidateDocUpdate = function() {
    /*jshint -W061 */
    const fn = fs.readFileSync('./webapp/src/ddocs/medic/validate_doc_update.js');
    eval('(' + fn + ')').apply(null, arguments);
  };
  ...
  it('allowed returns false on userCtx with null name', done => {
    assert.deepEqual(allowedOnServer({ name: null }, {}), disallowed('You must be logged in to edit documents'));
    done();
  });
  ...
  it('validateUserSettings fails if no name is defined', done => {
    ...
    assert.deepEqual(allowedOnClient(userCtx(), userSettings), disallowed('name property must be equivalent to username. e.g. "org.couchdb.user:sally"'));
    done();
  });

  it('validateUserSettings _id prefix must be org.couchdb.user', done => {
    userSettings._id = 'org.couchdb.foo:sally';
    assert.deepEqual(allowedOnClient(userCtx(), userSettings), disallowed('_id must be prefixed with "org.couchdb.user:". e.g. "org.couchdb.user:sally"'));
    done();
  });

  it('validateUserSettings _id must define a value after :', done => {
    userSettings._id = 'org.couchdb.user:';
    assert.deepEqual(allowedOnClient(userCtx(), userSettings), disallowed('_id must define a value after "org.couchdb.user:". e.g. "org.couchdb.user:sally"'));
    done();
  });
  ...
  it('validateUserSettings known must be boolean', done => {
    ...
    assert.deepEqual(allowedOnClient(userCtx(), userSettings), disallowed('known is not a boolean.'));
  ...
  it('validateUserSettings roles must exist', done => {
    ...
    assert.deepEqual(allowedOnClient(userCtx(), userSettings), disallowed('roles is a required array'));
    done();
  });
