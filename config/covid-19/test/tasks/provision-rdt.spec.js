...
const RDT_PROVISION_RUNNING = {
  provision: {
    'rdtoolkit-provision': { 'android-app-outputs': { rdt_session_bundle: { rdt_session_state: 'RUNNING' } } }
  }
};
const RDT_CAPTURE_QUEUED = {
  capture: {
    'rdtoolkit-capture': { 'android-app-outputs': { rdt_session_bundle: { rdt_session_state: 'QUEUED' } } }
  }
};
...
describe('Covid-19 RDT - Tasks', () => {
  ...
  it('Should show correct task when provision is submitted', async () => {
    const provisionRDT = await harness.fillForm(
      {
        form: 'covid19_rdt_provision',
        // Mock data from RDT by injecting content up-front
        content: RDT_PROVISION_RUNNING
      },
      ...formInputs.provisionRDT
    );
  ...
  it('Should resolve task when results are captured', async () => {
    ...
    const [task] = tasks;
    task.emission.actions[0].content = {
      // Mock data from RDT by injecting content up-front
      ...RDT_CAPTURE_QUEUED,
      ...task.emission.actions[0].content
    };
  ...
  it('Should not resolve task if capture form doesnt correspond with the provisioned RDT', async () => {
    ...
    const captureResult = await harness.fillForm({
      form: 'covid19_rdt_capture',
      // Mock data from RDT by injecting content up-front
      content: RDT_CAPTURE_QUEUED
    }, ...formInputs.captureResult);
  ...
  it('Should show task for repeating a provision RDT', async () => {
    ...
    expect(updatedTask.emission).to.include({
    ...
    expect(updatedTask.emission.actions[0]).to.include({
      type: 'report',
      form: 'covid19_rdt_provision'
    });
  });
