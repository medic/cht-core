...
module.exports = {
  ...
  checkOHWDuplicates: checkOHWDuplicates,
    ...
    */
    ...
    /*
     * Returns the first document matching the supplied id in the `patient_id` field
     */
    getRegistrations: function(options, callback) {
        var db = options.db,
            id = options.id,
            form = options.form || module.exports.getPatientRegForm();

        if (!form) {
            return callback('Please specify the registration form code.');
        }
    ...
    /*
     * return falsey value if validates otherwise return error message.
     */
    validatePatientId: function(id, validationFun) {

        var errors = [];

        if (typeof validationFun === 'function')  {
            var test = validationFun(id);
            if (test) {
                errors.push(mustache.to_html(test, {
                    patient_id: id,
                    id: id
                }));
                return errors;
        ...
        // at least check for existence and not 0
        if (id) return errors;

        // default error message
        return errors.push(mustache.to_html(
            "Patient ID '{{id}}' is invalid. Please correct this and try again.",
            {id: id}
        ));
