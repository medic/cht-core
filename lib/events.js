...
duality_events.on('init', function () {
    ...
    $('[data-dismiss=extended]').live('click', function(ev) {
    ...
    if (userCtx.name) {
        users.get(userCtx.name, function (err, user) {
            cookies.setBrowserCookie(dutils.currentRequest(), {
                name: 'KE_user_district',
                value: user.district || ''
            });
            
            if(user.district) {
                var link = $('#topnav .nav .records a');
                var urlparams = '&filter=' + user.district + '&startkey=["' + user.district + '"]';
                link.attr('href', link.attr('href') + urlparams);
            }
        });
    }
    else {
