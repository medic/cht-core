...
exports.addHelpers = function() {
    ...
    templates.addHelpers({
        ...
        contact: function(chunk, context, bodies, params) {
            ...
            if (params.messages) {
                var r = [];
                    messages = this.tapArray(params.messages, chunk, context);
                _.each(messages, function(task) {
                    _.each(task.messages, function(msg) {
                        r.push(msg.to);
                    });
                });
                // if more than two recipients then just display "x recipients"
                if (r.length > 2) {
                    contact = r.length + ' ' + $.kansotranslate('recipients');
                } else if (r.length > 0) {
                    contact = r.join(',');
                }
            }
