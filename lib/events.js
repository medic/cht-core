...
var subDataRecordChanges = function() {
    ...
    db.changes(opts, function(err, data) {
        if (err) return alert('Changes feed: '+err);
        if (!data || !data.results) return;
        //console.log('trigger data-record-updated',new Date().valueOf());
...
duality_events.on('init', function () {
    ...
    if (['chrome','safari','firefox'].indexOf(getBrowser()) === -1) {
        alert(
            'Warning Kujua does not support your browser.\n'
...
session.on('change', function (userCtx) {
    ...
    if (userCtx.name) {
        ...
        if (isAdmin) {
            $('.dropdown-menu .logout', el).before(
