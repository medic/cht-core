...
function getMessage(messages, locale) {
    ...
    if (!_.isArray(messages)) {
        console.warn('Message property should be an array. Upgrade your configuration to 0.4 compatibility.');
    } else {
...
module.exports = {
    ...
    addError: function(doc, error) {
        if (_.isString(error)){
            ...
            if (!error.message) {
                error.message = 'Failed to find a message in your validation settings.'
                    + '  This could be due to a locale mismatch.';
            }
        } else {
            error = {
                message: 'Message setting should be an object or string.'
                    + '  You may need to upgrade or modify your settings.'
            }
        }
