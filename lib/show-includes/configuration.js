...
var _submitConfiguration = function (_ev, _data, _validationFn) {
    ...
    if (_validationFn) {
        ...
        if (!valid.valid) {
            ...
            field.prepend('<div class="error-message help-block">' + valid.error + '</div>');
            return _updateStatus(form, { success: false, error: valid.error });
        }
    }
...
exports.workflow = function (_doc, _req) {
    ...
    events.once('afterResponse', function() {
        ...
        utils.updateTopNav('workflow', 'Schedule', ': ' + workflow.name);
...
console.log(_createReports(workflow));
    ...
    function _createValidationsModel (_validations) {
        var models = _.map(_validations.list, function(_validation) {
            return {
