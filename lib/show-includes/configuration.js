        ...
        shows.setupContext(_req, function() {
            ...
            var _validate = function (_data) {
                ...
                if (morning_hours > evening_hours || (
                    ...
                    errors.push({
                        ...
                        error: info.translate('The first time must be earlier than the second time')
                    });
                }
            ...
            var _uploadForms = function(_ev) {
                ...
                if (this.files.length === 0) {
                    finish(info.translate('File not found'));
                }
            ...
            utils.updateTopNav('settings', info.translate('Configuration'));
            ...
            $('#configuration-form').on('click', '.submit', function (_ev) {
                ...
                var data = {
                    locale: $('#locale').val(),
                    locale_outgoing: $('#locale-outgoing').val(),
                ...
                _submitConfiguration({
                    ...
                    if (_result.success) {
                        $('#default-language').html(
                            templates.render('configuration/default-language.html', _req, {
                                locales: data.locales,
                                locale: data.locale,
                                locale_outgoing: data.locale_outgoing
                            })
                        );
                        $('#language-to-edit').html(
                            templates.render('partials/locale.html', _req, {
                                locale: data.locale,
                                locales: data.locales
                            })
                        );
            ...
            $('#translations').on('click', '.submit', function (_ev) {
                ...
                _submitConfiguration({ event: _ev, data: { translations: translations }, info: info });
            });
...
exports.workflow = function (_doc, _req) {
    ...
    function _createScheduleModel (_workflow, info) {
        ...
        if (_workflow.registrations && _workflow.registrations.length) {
            fields = fields.concat(
                _getFields(_workflow.registrations[0])
            );
        }
        var messages = [{
            template: true,
