...
exports.workflow = function (_doc, _req) {
    ...
    function _getProperties(_report) {
        if (_report.properties) {
            return _report.properties;
        }
        return _.uniq(_.pluck(_report.validations.list, 'property'));
    }

    function _createValidationsModel (_report) {
        var properties = _getProperties(_report);
        return [{
            template: true,
    ...
    function _createScheduleModel (_workflow) {
        var properties = ['reported_date'];
        if (_workflow.registrations && _workflow.registrations.length) {
            properties = properties.concat(
                _getProperties(_workflow.registrations[0])
            );
        }
        var messages = [{
            template: true,
