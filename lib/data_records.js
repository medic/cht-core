...
var renderRecord = function(result) {
    ...
    var doc = result.doc || result,
        div = $('[rel='+ result.id || doc._id +']'),
        newDiv;

    console.log('renderRecord result',result);
    console.log('renderRecord doc',doc);
...
var handleAddMessage = function(ev) {
    ev.preventDefault();
    ...
    // setup modal
    $modal = $(templates.render('modal_add_message.html', {}, {
        to: getFacilityDesc(doc.related_entities.clinic)
    }));
    $modal.modal('show');
    $modal.on('click','.cancel', onModalCancel);
    $modal.on('click','.submit', onSubmit);
...
/*
 * include revs_info on doc
 */
function fetchDoc(id, rev, callback) {
    ...
    console.log('fetchdoc rev', rev);
    console.log('fetchdoc id', id);
    if (rev) q.rev = rev;
...
function updateRevNav(doc) {
    ...
    $div.find('.data-record-revisions .revision').text(doc._rev).show();

    _.each(revs_info, function(rev, idx) {
        if (doc._rev === rev.rev) {
            if (revs_info[idx-1]) {
                $fwd.removeClass('disabled');
                $fwd.data('rev', revs_info[idx-1].rev);
            } else {
                $fwd.addClass('disabled');
            }
            if (revs_info[idx+1]) {
                $back.removeClass('disabled');
                $back.data('rev', revs_info[idx+1].rev);
            } else {
...
var renderPrevRevision = function(doc) {
    _.each(doc._revs_info, function(rev, idx) {
        if (doc._rev === rev.rev) {
            if (doc._revs_info[idx+1]) {
                doc._rev = doc._revs_info[idx+1].rev;
                renderRecord(doc);
            }
        }
    });
...
var handleClickRevision = function(ev) {
    ...
    if (rev) {
        return fetchDoc(doc._id, rev, function(err, doc) {
    ...
    fetchDoc(doc._id, function(err, doc) {
        if (err) return alert('Error fetching document.',err);
        if ($btn.hasClass('icon-arrow-right')) {
            updateRevNav(doc);
        } else {
            renderPrevRevision(doc);
        }
    });
...
var setupListeners = function() {
    ...
    $(document).off('click', '.tasks-data_record .group-header .add-message');
    $(document).on('click', '.tasks-data_record .group-header .add-message', handleAddMessage);
...
var updateClinicsControl = function(query) {
    ...
    // unhide control
    $('#clinics-filter').show().find('[name=clinics]')
        .attr('placeholder', $.kansoconfig('Clinic Contact'))
        .val(q.cl_name);
    ...
    function update(err, rows) {
        ...
        $select.select2({
            allowClear: true,
            data: data,
            placeholder: $.kansoconfig('Clinic Contact')
        });
        $select.select2('val', q.cl_id);
        $select.on('change', function(e) {
            q.cl_id = e.val;
            q.cl_name = $select.select2('data').text;
...
var subDataRecordChanges = function() {
    var opts = {include_docs: true, filter:'kujua-base/data_records'};
