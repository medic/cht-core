...
function mapContactResult(row) {
    ...
    result = [doc.name, contact && contact.name, contact && contact.rc_code, contact && contact.phone];
...
function queryContacts(query) {
    ...
    if (query.byId || !/[\w\d]/.test(term)) {
        db.getView(ddoc, 'contacts_by_id', {
            include_docs: true,
            limit: 50,
