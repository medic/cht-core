...
// if the message doesn't have a document, create one and append
// related_entities if possible
...
exports.handleSendMessage = function(ev) {
    ...
    function onSubmit(ev) {
        ...
        exports.checkMessageDoc(doc, phoneData, function(doc) {
            ...
            updateInModalMode(function() {
                _.each(phoneData, function(data, idx) {
                    db.newUUID(100, function (err, uuid) {
                        ...
                        if (idx+1 === phoneData.length) {
                            db.saveDoc(doc, function(e, data) {
                                if (err) {
                                    handleModalError(e);
                                } else {
                                    console.log('saved doc. tasks are:', JSON.stringify(doc.tasks,null,2));
                                    setupModalClose([data]);
                                }
                            });
                        }
