...
exports.addListeners = function() {
    ...
    $(document).on('click', '.tasks-data_record .group-header .cancel', function(ev) {
        ...
        if (table.find('.new-task').length !== 0) {
            if (confirm('New rows were created, sure you want to remove them?'))
    ...
    $(document).on('click', '.tasks-data_record .remove-new', function(ev) {
        ev.preventDefault();
        var el = $(ev.target),
            table = el.closest('table');
        if (table.find('.new-task').length === 0)
            return alert('No new rows have been created.')
        else if (confirm('Remove new row?'))
            table.find('.new-task').last().remove();
    });
    ...
    $(document).on('click', '.tasks-data_record .group-header .save', function(ev) {
        ...
        db.getDoc(id, function(err, data) {
            ...
            tasks.each(function(idx, el) {
                ...
                val = task_field.find('.datepicker input').val();
                orig = task_field.find('[data-orig-val]').attr('data-orig-val');
                if (!val) {
                ...
                if (!val) {
                    ...
                    task_field.find('.control-group').addClass('error');
                    task_field.on('focus', '.control-group', function(ev) {
                        // for some reason bootstrap does not clear error
