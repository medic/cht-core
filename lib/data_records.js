...
var highlightRows = function(divs, bool) {
    ...
    for (var i in divs) {
        bool ? divs[i].addClass('warning') : divs[i].removeClass('warning');
    }
};

// remove more than one doc, confirm should prompt with total number of docs to
// be deleted.
var removeDocs = function(docs, callback) {

    console.log('calling removeDocs');
    var total = docs.length;

    if (docs.length === 0)
        return callback('No rows selected.');

    if(confirm('Permanently remove '+total+' records?')) {
        var rev, id;

        for (var i in docs) {
            id = docs[i]['_id'];
            rev = docs[i]['_rev'];
            db.removeDoc({_id: id, _rev: rev}, function(err, resp) {
                if (err)
                    return callback(err);
                // on screen delete happens with changes feed sub
            });
        };
    } else {
        callback(null, false);
...
function editRow(ev) {
    ev.preventDefault();
    ev.stopPropagation();
    var id = $(this).parents('.data-record').attr('rel');
    db.getDoc(id, {}, function(err, doc) {
        if (err)
            return alert('Error fetching doc id '+id+': '+err);
        var options = {data:doc};
        if (district)
...
exports.addListeners = function() {
    ...
    var handleEditDelete = function(ev) {
        ev.preventDefault();
        console.log('handleEditDelete(ev)');
        var docs = [],
            divs = [];
        ...
        $('#data-records [name=select-row-checkbox]:checked').each(function(idx, el) {
            ...
            docs.push({ _id: record._id, _rev: record._rev });
        });

        // highlight row to warn user
        //highlightRows(divs);

        removeDocs(docs, function(err, bool) {
            console.log('calling removeDocs callback');
            if (err)
                return alert(err);
            // cancelled
            //if (!bool) {
            //    highlightRows(divs, false);
            //}
        });
    }
