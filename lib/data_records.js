...
// optionally pass in form definition
var prettyVal = function(data_record, key, def) {

    if (data_record[key] === undefined) { return; }

    def = def ? def : jsonforms[data_record.form];

    if (!def) { return; }

    if (def.fields[key]) {
        switch (def.fields[key].type) {
            case 'boolean':
                if (data_record[key] !== undefined) {
                    return data_record[key] === true ? 'True' : 'False';
                }
            case 'date':
                if (data_record[key]) {
...
var fieldsToHtml = function(keys, labels, data_record) {
    ...
    _.each(keys, function(key) {
        if(_.isArray(key)) {
            ...
            fields.data.push({
                isArray: false,
                value: prettyVal(data_record, key)
            });
        }
    });
...
var renderRecords = function() {
    ...
    var render = function(err, data) {
        ...
        $('#data-records .data-record').each(function(index, row) {
          $(row).data('data-record', rows[index]);
        });
        delete timeoutID;
        $('.ajax-loader').hide();
    };
