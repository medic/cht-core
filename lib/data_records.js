...
/*
 * If the message doesn't have a document, create one and assigns
 * facility data.
 */
exports.checkMessageDoc = function(doc, facilities, callback) {
    var user,
        name,
        phone;
    ...
    if (doc) {
        ...
        /*
         * Only assigning first recipient facility to record related_entities
         * since that property determines who can view the record. Not sure how
         * to handle the record's permissions with a mult-recipient case yet.
         * Mabye reduce the recipients list into the most common parent.
         ...
         */
        var facility = _.first(facilities);
        if (facility && facility.id) {
            db.getDoc(facility.id, function(err, contact) {
                doc.related_entities = {};
                doc.related_entities[contact.type] = contact;
                callback(doc);
            });
        } else {
            callback(doc);
...
exports.handleSendMessage = function(ev) {
    ...
    function onSubmit(ev) {
        ...
        exports.checkMessageDoc(doc, recipients, function(doc) {
            updateInModalMode(function() {
