...
function updateClinicsControl(query) {
    ...
    view_q = {
        startkey: [district || null],
        endkey: [district || null, {}]
    };
    ...
    configureControl({
        ...
        data: data,
        placeholder: 'Contact',
        display: q.dh_name || q.cl_name,
        onSelectChange: function(e) {
            var data = $select.select2('data'),
                removeKeys = ['patient_id'],
                idKey,
                textKey;
            ...
            if (data) {
                if (data.type === 'clinic') {
                    idKey = 'cl_id';
                    textKey = 'cl_name';
                    removeKeys.push('dh_id', 'dh_name');
                } else if (data.type === 'district_hospital') {
                    idKey = 'dh_id';
                    textKey = 'dh_name';
                    removeKeys.push('cl_id', 'cl_name');
                } else {
                    removeKeys.push('dh_id', 'dh_name', 'cl_id', 'cl_name');
                }
            } else {
                removeKeys.push('dh_id', 'dh_name', 'cl_id', 'cl_name');
            }
    ...
    getCachedView(view, {
        ...
        $select.select2('val', q.dh_id || q.cl_id);
