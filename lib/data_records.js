...
function onDataRecordUpdated(ev, data) {
...
function handleSendMessage(ev) {
    ...
    $modal = $(templates.render('modal_send_message.html', {}, {
        to: getFacilityDesc(doc && doc.related_entities.clinic)
    }));
    $modal.modal('show');
    $modal.on('click','.cancel', onModalCancel);
    $modal.on('click','.submit', onSubmit);
...
var setupListeners = function() {
    ...
    $(document).off('click', '#data-records .main, #data-records .message');
    $(document).on('click', '#data-records .main, #data-records .message', function(ev) {
        if ($(ev.target).closest('button').length === 0) {
            ev.preventDefault();
            $(this).next('.extended').toggle('hide');
        }
    });
    ...
    $(document).off('click', '.add-message button');
    $(document).on('click', '.add-message button', handleSendMessage);
...
var updateClinicsControl = function(query) {
    ...
    function update(err, rows) {
        ...
        function onSelectChange(e) {
            if (e.val && e.type !== 'select2-removed') {
                q.cl_id = e.val;
        ...
        $select.on('select2-removed', onSelectChange);
    }
...
var updateFormsControl = function(query) {
    ...
    if (form === '*') {
        ...
        label = _s.sprintf('Form: <b>%s</b>', form);
    } else if (form === null) {
        label = 'Messages Only';
    } else {
        label = 'Form';
    }
    $('#form-filter').show().find('a.btn')
        .html(_s.sprintf('%s <span class="caret"></span>', label));
    // update the show all link
    q = _.omit(q, 'form');
    $('.dropdown-menu.forms a').attr('href', baseURL + '/data_records?' + querystring.stringify(q));
    ...
    function update(err, rows) {
        ...
        $dd.append(li_template(_.extend(q, {form: 'null'}), 'Show Messages', baseURL));
        $dd.append(li_template(_.extend(q, {form: '*'}), 'Show Forms', baseURL));
