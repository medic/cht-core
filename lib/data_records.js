...
exports.addListeners = function() {
    ...
    // schedule editing
    //
    $(document).on('mouseenter', '.tasks-data_record .group-header', function(ev) {
        var el = $(ev.target);
        if (el.closest('table').hasClass('edit-mode'))
            return;
        el.find('.btn-group.menu').show();
    });
    $(document).on('mouseleave', '.tasks-data_record .group-header', function(ev) {
        var el = $(ev.target);
        if (el.closest('table').hasClass('edit-mode'))
            return;
        el.find('.btn-group.menu').hide();
    });
    $(document).on('click', '.tasks-data_record .group-header .edit', function(ev) {
        ev.preventDefault();
        var el = $(ev.target);
        el.closest('table').addClass('edit-mode')
    ...
    $(document).on('click', '.tasks-data_record .group-header .save', function(ev) {
        ...
        var button = $(ev.target),
            ...
            table = $(ev.target).closest('table');

        // disable while we do IO
        button.attr('disabled', 'disabled');

        function finalize(err, updated) {
            button.removeAttr('disabled');
            if (err || !updated) return alert(err);
        ...
        db.getDoc(id, function(err, data) {
            ...
            var tasks = $(ev.target).closest('table').find('[data-tasks-idx]'),
                updated = false;
            tasks.each(function(idx, el) {
                var task = $(el),
                    task_idx = task.attr('data-tasks-idx'),
                    task_field = task.find('.tasks-state [data-save-val]'),
                    task_val = task_field.attr('data-save-val'),
                    task_orig = task_field.attr('data-orig-val');
                if (task_val && data.scheduled_tasks[task_idx].state !== task_val) {
                    data.scheduled_tasks[task_idx].state = task_val;
                    updated = true;
                }
            });
            if (updated) {
                db.saveDoc(data, function(err, data) {
                    if (err) return finalize('Save failed: ' + err);
                    return finalize(null, updated);
                    //once doc is saved changes listener will update the row
                });
            } else {
                finalize('Cancelled save, values did not change.');
            }
        });
    });
    $(document).on('click', '.tasks-data_record .group-header .cancel', function(ev) {
        ...
        table.removeClass('edit-mode').find('.btn-group.menu').show();
    });
    $(document).on('click', '.tasks-data_record .tasks-state .dropdown-menu a', function(ev) {
        ev.preventDefault();
        var info,
            el = $(ev.target),
            changeEl = el.closest('.wrap').find('.label'),
