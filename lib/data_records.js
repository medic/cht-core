...
var setupInboxListeners = function() {
    ...
    $('#edit-user-profile .submit').on('click', function(e) {
        ...
        userDb.getDoc('org.couchdb.user:' + user, function(err, updated) {
            var password = $('#edit-user-profile #password').val();
            if (password) {
                if (password !== $('#edit-user-profile #password-confirm').val()) {
                    enableModal();
                    $modal.find('.modal-footer .note').text('Passwords must match');
                    return;
                }
    ...
    var init = function() {
        $('body').trigger('dataRecordsInitialized', { district: district });
    };
    // Race condition between inbox.js and this.
    // Fire init now in case this is executed last.
    init();
    // Fire init later in case this is executed first.
    $('body').on('inboxInitialized', init);
};
...
var inboxQuery = function(options, callback) {
    ...
    if (options.changes) {
        var updated = _.filter(options.changes.results, function(result) {
            return !result.deleted;
        });
        var updatedIds = _.map(updated, function(result) {
            return '"' + result.id + '"';
        });
        if (!updatedIds.length) {
            return callback(null, { rows: [] });
        }
        options.q += ' AND uuid:(' + updatedIds.join(' OR ') + ')';
    }
    db.getFTI(
        ddoc,
        'data_records',
...
exports.init = function(req, options) {
    ...
    reporting.init(req, options);
    if ($('body').is('.inbox')) {
        setupInboxListeners();
    }
    subDataRecordChanges();
    if ($('#analytics-tab').is('.selected')) {
        reporting.render_page();
    }
