...
var onDataRecordUpdated = function(ev, data) {
    ...
    //console.log('onDataRecordUpdated', new Date().valueOf());
    ...
    finishDataRecordUpdate();
};

var finishDataRecordUpdate = function() {
    closeModal();
};
...
var setupListeners = function() {
    ...
    var handleMarkInvalid = function(ev) {
        ...
        // records are updated it can take a while.
        var runUpdate = function(callback) {
            orig_text = $modal.find('.submit').text();
        ...
        var handleError = function(err) {
            $modal.find('.submit').text(orig_text);
        ...
        var handleSubmit = function(ev) {
            ev.preventDefault();
            var $el = $(this),
                msg = $modal.find('textarea').val().trim();

            if (!msg) {
                $modal.find('.control-group').addClass('error');
                $modal.find('.error-msg').show();
                return;
            }
            for (var i in docs) {
                docs[i].errors.push({
        ...
        doBulkEdit(options, function(err, _docs) {
            ...
            if(confirm('Update '+_docs.length+' records?')) {
                // setup modal
                $modal = $(templates.render('edit_invalid.html', {}, {}));
                $modal.modal('show');
                $('#invalid-reason').focus();
                $modal.on('click','.cancel', handleCancel);
                $modal.on('click','.submit', handleSubmit);
                docs = _docs;
            }
        });
    }
    ...
    $(document).on('click', '.controls .edit-mode .edit-update', handleEditUpdateFacility);
    ...
    $(document).on('click', '.tasks-data_record .tasks-state .dropdown-menu a');
