...
//
var getViewForFilter = function() {

    // always check for admin or enforce district filtering.
    if (!isAdmin && !district) return viewName;
    if (!isAdmin && !dh_id && !district) return viewName;

    // no filters are set
    if (!hasForm && !valid && !patient_id && !clinic_id)
        return dh_id ?
...
exports.init = function(req, options) {
    ...
    hasForm = options.form === "null" || options.form,
    form = options.form === "null" ? null : options.form,
    valid = options.valid,
    dh_id = district ? district : options.dh_id,
    ...
    if (clinic_id) {
        q.startkey.unshift(clinic_id);
        q.endkey.unshift(clinic_id);
    }
    if (dh_id) {
