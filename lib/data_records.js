...
function render(err, data) {
    ...
    if (data.rows && data.rows.length === 1) {
        ...
        nextStartRow = data.rows.pop();
    }
    var rows = _.map(data.rows, function(row) {
        return sms_utils.makeDataRecordReadable(row.doc || row.value, sms_utils.info);
    });
...
function renderRecords(options) {
    ...
    if (nextStartRow) {
        options.query['startkey'] = nextStartRow.key;
        options.query['startkey_docid'] = nextStartRow.id;
    }
...
var renderRecord = function(result) {
    ...
    var html = templates.render( 'data_records_rows.html', {}, {
        data_records: [
            sms_utils.makeDataRecordReadable(doc, sms_utils.info)
        ]
    });
...
function handleUpdateFacility(ev) {
    ...
    doBulkEdit({}, function(err, _docs) {
        ...
        $modal.modal('show');
        var el = $modal.find('[name=facility]');
        configureControl({
            allowClear: false,
            el: el,
            data: data,
        ...
        db.getView(ddoc, view, q, function(err, result) {
            ...
            el.parent().find('.select2-container .select2-default span').html(
                $.kansotranslate('Select facility')
            );
...
var handleScheduleGroupUpdate = function(ev) {
    ...
    var onSubmit = function(ev) {
        ...
        tasks.each(function(i, el) {
            ...
            utils.setTaskState(doc.scheduled_tasks[idx], state_val);
        });
        updateInModalMode(function() {
...
exports.countChars = function($message, $modal) {
    ...
    if (count > 50) {
        $modal.find('.modal-footer .note').html(
            count + '/160 ' + $.kansotranslate('characters')
        );
    } else {
        $modal.find('.modal-footer .note').html('');
    }
...
var handleScheduleGroupSave = function(ev) {
    ...
    tasks.each(function(idx, el) {
        ...
        if (val && doc.scheduled_tasks[task_idx].state !== val) {
            utils.setTaskState(doc.scheduled_tasks[task_idx], val);
            updated = true;
        }
    ...
    for (var i = 0; i < new_tasks.length; i++) {
        ...
        var to = new_task.find('.tasks-to [name=phone]'),
            recipients = exports.validatePhoneNumbers(to.select2('data')),
        ...
        if (!recipients.valid) {
            ...
            return finalize(recipients.message);
        }
        if (!msg_val) {
        ...
        var messages = _.map(recipients.value, function(recipient) {
            return {
                to: recipient.phone,
                message: msg_val
            };
...
function renderRevision(id, rev, callback) {
    fetchDoc(id, rev == 'current' ? undefined : rev, function(err, data) {
        if (!err) {
            renderRecord({
                doc: data,
                id: data._id
...
var updateMessage = function(messageId, updateFn) {
    db.getDoc(messageId, function(err, message) {
        ...
        updateFn(message, function(err, updated) {
            ...
            updateInModalMode(function() {
                db.saveDoc(updated, function(err, data) {
                    if (err) {
                        stopDataChangesListener();
                        return console.log(err);
                    }
...
function setupListeners() {
    ...
    $document.on('click', '.tasks-data_record [type=submit]', function(ev) {
        ...
        var btn = $(this),
            ...
            recipients = exports.validatePhoneNumbers(input.select2('data'));
        if (!recipients.valid) {
        ...
        db.getDoc(id, function(err, data) {
            ...
            var msg_val = data.tasks[tasks_idx].messages[idx].message;
            var messages = _.map(recipients.value, function(recipient) {
                return {
                    to: recipient.phone,
                    message: msg_val
            ...
            data.tasks[tasks_idx].messages = messages;
            utils.setTaskState(data.tasks[tasks_idx], 'pending');
            data._rev = rev; // set the rev from the form

            // remove resolved errors
            data.errors = _.filter(data.errors, function(error) {
                // legacy error messages
                return e.error !== 'Could not find message recipient.'
                    && !(e.code && e.code.match(/recipient_not_found/));
            });
    ...
    $document.on('click', '.msg-full .recipient, .msg-full .sent_by', function(ev) {
        // toggle short/long recipient description
        ev.preventDefault();
        var $btn = $(ev.target).closest('a');
        if ($btn.hasClass('short')) {
            $btn.hide().prev().show();
        } else {
            $btn.hide().next().show();
        }
    ...
    /*
     * clear popovers (if we use them again)
     *
...
function getViewQuery(req) {
    ...
    var dh_id = district ? district : req.query.dh_id;
    ...
    getCachedView(view, {
        ...
        _.each(rows, function(row) {
            ...
            if (key !== 'null') {
                data.push({
                    id: key,
                    text: sms_utils.getFormTitle(key, sms_utils.info)
                });
            }
        });
        data.sort(sortSelect2Data);
...
exports.init = function(req, options) {
    ...
    if (!options.inbox) {
        ...
        if (currentRequest) {
            console.log('already processing request, skipping.');
            return updateControls(req, function(){});
        } else {
            currentRequest = req;
