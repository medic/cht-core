...
const INITIAL_ID_LENGTH = 5,
      ID_LENGTH_DOC_ID = 'shortcode-id-length',
    ...
    db.medic.put(idLengthDoc, err => {
      if (err) {
        ...
        if (err.statusCode === 409) {
          console.log(`409 while trying to store ${idLengthDoc}. If someone edited this document it is expected.`);
        } else {
          return reject(err);
        }
      }
...
const generateNewIds = (currentIdLength) => {
  console.log('342314', currentIdLength);
