...
module.exports = {
    /*
     * Return the value on the doc defined by a string.  Support dot notation
     * so the `start_from` configuration can support nested properties.
     */
    _getVal: function(doc, key) {
        var tree = {},
            val;
        (key || '').split('.').forEach(function(k) {
          if (typeof doc[k] !== 'undefined') {
            tree = doc[k];
            val = doc[k];
          } else if (typeof tree[k] !== 'undefined') {
            val = tree[k];
            tree = tree[k];
          }
        });
    ...
    assignSchedule: function(doc, schedule) {
        ...
        docStart = self._getVal(doc, schedule.start_from);
