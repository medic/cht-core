...
var getReferralRecipient = function(form, clinic, from) {
    switch (form) {
        case 'MSBR':
 ...
 * @param {Object} form_data - parsed smsform data that includes labels
 * (format:1)
 *
 ...
 * @api private
 */
var getReferralMessage = function(phone, form, form_data) {
    var ignore, message = [];
    switch (form) {
        case 'MSBC':
            if (isFromHealthCenter(phone, clinic))
...
exports.tasks_referral = function (head, req) {
    ...
    task.messages.push({
        to: task.to,
        message: getReferralMessage(task.from, task.form, task.form_data)});
