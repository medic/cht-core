...
/*
 * Constructs tasks_referral doc and includes it in response as callback data
 * for gateway to execute. A tasks_referral doc combines a sms_message and
 * facility so that messages can be sent to different facilities.
 */
exports.tasks_referral = function (head, req) {
    ...
    if (!req.query.data) {
        return '{' +
            '"error": "Please include a data argument with the sms_message."}';
    }

    var task = null; //JSON deals better with null than undefined

    var sms_message = JSON.parse(req.query.data),
        form_key = sms_message.form,
        def = smsforms[form_key];
    ...
    task = {
        ...
        form: smsparser.parse(def, sms_message, 1),
        facility: null
    }

    var row = {};
    while (row = getRow()) {
        task.facility = row.value;
        break;
    }
