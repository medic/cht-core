...
module.exports = {
    ...
    extra_validations: {
        ...
        unique: function(doc, fields, callback) {
            console.log('extra_validations.unique() arguments', arguments);
    ...
    /*
     * Given Pupil.js parsed token return String or Number values.
     */
    tokenVals: function(token) {
        var vals = [];
            type = token.name.name;
        if (type === 'String' || type === 'Number') {
            vals.push(token.data);
        }
        console.log('tokenVals returning', vals);
        return vals;
    },
    /**
     ...
     *
     * @callback Array of errors if validation failed, falsey value otherwise.
     * */
