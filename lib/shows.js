...
var querystring = require('querystring'),
    ...
    ddoc = settings.name,
    muvuku_webapp_url = '/json-forms/_design/json-forms/_rewrite/',
    district,
    isAdmin,
    isDistrictAdmin;
...
function onDownloadFormSubmit(ev) {
    var $target = $(ev.currentTarget),
        ...
        format = $('#forms [name=format]').val().split(','),
...
var renderDownloadForms = function(err, data) {
    ...
    // FIXME Temporary to stop unknown forms showing when they have no data
    keys = _.without(_.unique(keys), 'Unknown');
    keys.sort();
...
exports.sms_forms = function (doc, req) {
    ...
    return {
        title: 'SMS Forms',
        muvuku_webapp_url: getMuvukuWebappUrl.apply(this),
 ...
 */
var getMuvukuWebappUrl = function() {
    //if (this.app_settings && this.app_settings.muvuku_webapp_url) {
    if (this.app_settings && this.app_settings.muvuku_webapp_url) {
        muvuku_webapp_url = this.app_settings.muvuku_webapp_url;
    }
    return muvuku_webapp_url;
}
