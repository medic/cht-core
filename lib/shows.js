...
exports.users = function (_doc, _req) {
            ...
            results: function (_data, _page) {
                ...
                _.each(_data.rows, function (_row) {
                    ...
                    if (!_.isObject(map[type])) {
                        map[type] = {
                            children: [],
                            text: info.translate(_str.titleize(_str.humanize(type)))
                        };
                    }
    ...
    var populate_users = function (_elt, _users, _facilities) {
        get_admins(function(err, admins) {
            ...
            var model = _.map(_users, function(user) {
                ...
                var facility = (_facilities[user.facility_id] || {}).name;
    ...
    events.once('afterResponse', function() {
        ...
        setupContext(_req, function() {
            ...
            $('#language').select2({ allowClear: true });

            select_facility_grouped('#facility_id');

            utils.updateTopNav('user-management', info.translate('Users'));
            ...
            $('#add-user').on('click', function (_ev) {
                _ev.stopPropagation();
                _ev.preventDefault();
                _showModal(false, info.translate('Add User'));
            });
            ...
            userManagementModal.on('click', '.submit', function (_ev) {
                ...
                if (!user.name) {
                    field.push('#name');
                    error.push(info.translate(
                        'field is required', { field: info.translate('User Name') }
                    ));
                }
                if (!user._id && !user.password) {
                    field.push('#password');
                    error.push(info.translate(
                        'field is required', { field: info.translate('Password') }
                    ));
                }
                var passwordConfirm = $('#password-confirm').val();
                if ((user.password || passwordConfirm) && user.password !== passwordConfirm) {
                    field.push('#password');
                    error.push(info.translate('Passwords must match'));
                }
                if (error.length) {
            ...
            $('#user-list tbody').on('click', '.edit', function (_ev) {
                _ev.stopPropagation();
                _ev.preventDefault();
                _showModal(_getUser(_ev), info.translate('Edit User'));
            });
            ...
            $('#user-list tbody').on('click', '.delete', function (_ev) {
                ...
                $('#user-delete-confirmation-label')
                    .text(info.translate('Confirm delete user', { name: user.name }));
