...
exports.workflow = function (_doc, _req) {
    ...
    function _formatDate (_form) {
        if (!_form || !_form.silence_for) {
            return {
                value: '',
                unit: 'days'
            };
        }
        var parts = _form.silence_for.split(' ');
        return {
            value: parts[0],
            unit: parts[1]
        };
    ...
    var visitSilence = _formatDate(visit);
    var delivery = _findForm(info.patient_reports, 'D');
    var deliverySilence = _formatDate(delivery);
    ...
    var messages = [];
    _.each(schedules, function (_schedule) {
        _.each(_schedule.messages, function (_message) {
            // TODO use selected language
            messages.push({
                schedule: _schedule,
                message: _message,
