...
exports.data_records = function(doc, req) {
    ...
    sms_utils.info = appinfo.getAppInfo.call(this, req);
    ...
    return {
        info: sms_utils.info,
        title: sms_utils.info.translate('Activity'),
...
exports.inbox = function(doc, req) {
    ...
    var updateRows = function() {
        ...
        if (query) {
            options.q = query;
            data_records.query(req, options, function(err, rows) {
                if (err) {
                    console.log(err);
                } else {
    ...
    events.once('afterResponse', function() {
        // TODO This will be registered multiple times?
        $('body').on('updateMessages', updateRows);
    });

    return {
        html: templates.render('inbox.html', req, {
            info: sms_utils.info
        })
    };
};
    ...
    var transforms = [{
        ...
        up: function(settings) {
            ...
            _.each(settings.registrations, function(registration) {
                ...
                registration.messages = _.map(registration.messages, function(message) {
                    return {
                        message: [{
                            content: message.message,
            ...
            _.each(settings.schedules, function(schedule) {
                _.each(schedule.messages, function(message) {
                    message.message = [{
                        content: message.message,
                        locale: message.locale
                    }];
