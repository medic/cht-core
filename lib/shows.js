...
var uploadFacilities = function(ev) {
  ...
  reader.onloadend = function(ev) {
       ...
       async.forEach(json, function(facility, cb) {
          db.saveDoc(facility, function(err) {
              if (err) {
                 console.error('error saving facility', err, facility);
                 errors.push(facility.name);
              } else {
                 load_count++;
                 $('#facilities-controls .restore').html('Restored '+load_count);
              }
              cb();
          });
       }, function(err){
           if (err) return alert('Error loading: ' + err);
           var msg = 'Load complete. ' + load_count + ' facilities loaded. ';
           if (errors.length > 0) {
              msg += 'The following facilities did not load: ';
...
var renderFacilitiesControls = function() {
    ...
    $('#facilities-controls .restore').on('click', restoreFacilities);
