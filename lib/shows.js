...
function renderDownloadMessages(err, data) {
    ...
    if (isAdmin) {
        ...
        _.each(data.rows, function(row) {
            ...
            messages.districts.push({
                ...
                q: querystring.stringify(db.stringifyQuery({
                    startkey: startkey.unshift(dh_id),
                    endkey: endkey.unshift(dh_id),
                    dh_name: dh_name,
                    tz: tz
                }))
            });
        ...
        messages.q = querystring.stringify(db.stringifyQuery({
            startkey: startkey.unshift(district),
            endkey: endkey.unshift(district),
            tz: tz
        }));
    }
