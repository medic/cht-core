...
var removeFacility = function(audit, doc, callback) {
    audit.removeDoc(doc, function (err, res) {
        if (err) {
            return callback(err);
        }
        callback(null, doc);
...
var updateChildFacilities = function(doc) {
    ...
    db.getView(ddoc, view, q, function(err, data) {
        ...
        for (var i in data.rows) {
            ...
            d.parent = doc;
            audit.saveDoc(d, function(err, res) {
                if (err)
                ...
                if (currentFacilityRequest === req) {
                    ...
                    $('[data-page=facilities] #facilities').spreadsheet({
                        ...
                        save: function (doc, callback) {
                            ...
                            audit.saveDoc(doc, function (err, res) {
                                if (err) {
                                    return callback(err);
                                }
                                doc._rev = res.rev;
                        ...
                        remove: function(doc, callback) {
                            removeFacility(audit, doc, callback);
                        },
