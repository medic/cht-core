...
exports.sms_forms = function (doc, req) {
    ...
    events.on('afterResponse', function() {
        ...
        db.getView('kujua-export', 'data_record_form_keys', {group: true}, function(err, data) {
            if (err) {
                return alert(err);
            }
            
            var forms = _.map(data.rows, function(row) {
                return {form: row.key[0], title: row.key[1], size: row.value};
            });
...
var getUserDistrict = function(userCtx) {
...
var updateControls = function(userCtx) {
    ...
    if(!isUserAdmin(userCtx)) {
        return;
    }

    var q = {group: true};

    db.getView('kujua-export', 'district_hospitals_from_data_records', q, function(err, data) {
...
exports.data_records = function(head, req) {
    ...
    events.once('afterResponse', function() {
        ...
        // each request.
        var associated_district = getUserDistrict(req.userCtx),
            isAdmin = isUserAdmin(req.userCtx),
        ...
        if (isAdmin || associated_district) {
            var q = isAdmin ? {} : {startkey: [associated_district]};
            console.log(['q',q]);
            db.getView(
                'kujua-export',
                'data_records_by_reported_date', q, renderRecords);
        } else {
             renderRecords(null, []);
        }
    });
