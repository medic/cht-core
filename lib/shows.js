...
function renderDownloadMessages(err, data) {
     ...
     */
    if (isAdmin) {
        // populate messages.districts property if admin
        ...
        messages = { districts: _.sortBy(districts, 'dh_name') };
    } else if (isDistrictAdmin) {
        messages = { dh_id: district };
    }
...
var render403 = exports.render403 = function(req, msg) {
    if ($('body').is('.inbox')) {
        if (msg) {
            // error with user setup
            $('body').html(templates.render('403.html', req, {
                msg: msg
            }));
        } else {
            // not logged in
...
var setupContext = function(req, callback) {
    ...
    if (isDistrictAdmin) {
        exports.checkDistrictConstraint({
            ...
            if (err) {
                return render403(req, err);
            }
            return callback();
        });
    } else {
        return render403(req, 'You must be an admin or a district admin to access this page. Your admin needs to update your permissions.');
    }
};

exports.checkDistrictConstraint = function(options, callback) {
    utils.getUserDistrict(options.userCtx, function(err, district) {
        if (err) {
            return callback(err);
        }
        if (!district) {
        ...
        options.db.getDoc(district, function(err, doc) {
            if (err) {
                if (err.error === 'not_found') {
                    return callback("No facility found with id '" + district + "'. Your admin needs to update the Facility Id in your user details.");
                }
                return callback(err);
            }
            if (doc.type !== 'district_hospital') {
                return callback("Facility with id '" + district + "' is not a district hospital. Your admin needs to update the Facility Id in your user details.");
            }
            callback();
        });
    });
...
exports.users = function (_doc, _req) {
    ...
    var user_properties = [
        '_id', '_rev', 'name', 'fullname', 'email',
        'password', 'phone', 'language', 'facility_id',
        'salt', 'derived_key', 'password_scheme', 'iterations'
    ];
