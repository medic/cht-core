...
exports.makeDataRecordReadable = function(row, locale) {
...
var renderRecords = function(err, data) {
    ...
    var rows = _.map(data.rows, function(row) {
        var r = exports.makeDataRecordReadable(row);
        r._key = row.key;
        return r;
    });
...
var dataRecords = {
    ...
    renderRecords: function() {
        logger.debug('firing renderRecords');
        var self = this,
            q = self.viewQuery;
    ...
    addListeners: function() {
        ...
        $(document).on('click', '.tasks-referral [type=submit]', function(ev) {
            ...
            var btn = $(this),
                ...
                taskIdx = parseInt(btn.closest('table').attr('data-tasks-idx'), 10),
            ...
            self.db.getDoc(id, function(err, data) {
                if (err) { return alert(err); }
                data.tasks[taskIdx].messages[idx].to = phone;
                data.tasks[taskIdx].state = 'pending';
                ...
                self.db.saveDoc(data, function(err, data) {
                    if (err) { return alert(err); }
                    if(!data.ok) { return alert('saveDoc failed: ' + data); }
                    // set the to value in the page
                    btn.closest('li.to').html('To: '+phone);
                    $('[rel='+data.id+'] td.errors').html(
                        '<i class="pending icon-ok icon-large"></i>');
                    if(!errors.length) {
                        $('[rel='+data.id+'] .data-record-errors').fadeOut().remove();
                    }
                    form.remove();
                    logger.debug(data);
                });
    ...
    subChanges: function() {
        //this.db.changes(function(err,data){ console.log([err,data]);})
    },
