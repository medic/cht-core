...
var updateControls = function(req) {
    ...
    //
    // show filters and highlight filter buttons to show active filter
    //
    ...
    if (isAdmin) {
        ...
        $('.dropdown-menu.district-hospitals a').attr(
            'href', baseURL + '/data_records?' + querystring.stringify(q2)
        );
    ...
    // check for patient ids before displaying patient-id filter
    var view = dh_id ?
...
exports.data_records = function(head, req) {
    ...
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            data_records.init(req, district, isAdmin, dh_id, form, valid, patient_id);
            updateControls(req);
