...
var updateControls = function(userCtx) {
    ...
    db.getView('kujua-export', 'data_records_valid_by_district_and_form',
        {group: true}, function(err, data) {
            if (err) { return alert(err); }

            var forms = _.uniq(_.compact(_.map(data.rows, function(row) {
                return row.key[1];
            })));
    ...
    db.getView('kujua-export', 'data_records_by_district', 
        {group: true}, function(err, data) {
            if (err) { return alert(err); }
