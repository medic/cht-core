...
exports.users = function (_doc, _req) {
    ...
    var populate_user = function (_elt, _user) {
        ...
        $('#name').val(_user.fullname);
        $('#facility').select2('val', _user.facility_id);
        $('#user').val(_user._id.replace(/^org.couchdb.user:/, ''));
    ...
    var serialize_user = function (_elt) {
        ...
        var facility_id = $('#facility').select2('val');

        if (facility_id) {
            rv.facility_id = facility_id;
        }
     ...
     * save_user:
     */
    var save_user = function (_user) {

        if (_user._id) {
            console.log('update', _user);
            return;
        }
    ...
    var populate_users = function (_elt, _users, _facilities) {
        ...
        _.each(_users, function (_user, _i) {
            ...
            var facility_label = (
                (_facilities[facility_id] || {}).name || 'Unknown'
            );

            input.attr('id', _user._id);

            tr.append(td.clone().append(input))
              .append(td.clone().text(_user.name))
    ...
    events.once('afterResponse', function() {
        ...
        /* Fix me: Check errors here */
        fetch_facilities(function (_err, _facilities) {
            fetch_users(function (_err, _users) {
                populate_users('#user-rows', _users, _facilities);
            });
        });

        $('h1.pull-left').append('User Management');

        $('#save').click(function (_ev) {
            save_user(serialize_user('#user-creation'));
        });
    });
