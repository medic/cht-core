...
exports.workflow = function (_doc, _req) {
    ...
    var workflows = {
        'basic': {
            ...
            overview: 'workflow/workflow_basic_overview.html',
            incoming: 'workflow/workflow_basic_incoming.html',
            outgoing: 'workflow/workflow_basic_outgoing.html'
        }
    };
    ...
    events.once('afterResponse', function() {
        ...
        var _mergeMessages = function (_messages, _message) {
            var locale = $('#language').val();
            _messages = _messages ? JSON.parse(_messages) : [];
            var message = _.find(_messages, function(_m) {
                return _m.locale === locale;
            });
            if (message) {
        ...
        var _getValidations = function (_elem) {
            ...
            _elem.find('.validation').each(function() {
                var messages = $(this).find('[name=messages]').val();
                var newMessage = $(this).find('[name=content]').val();
                validations.push({
                    property: $(this).find('[name=property]').val(),
                    rule: $(this).find('[name=rule]').val(),
                    message: _mergeMessages(messages, newMessage)
                });
            });
            return validations;
        };
        ...
        var _updateRegistration = function (_data) {
            ...
            var messages = elem.find('.responses [name=messages]').val();
            var newMessage = elem.find('.responses [name=content]').val();
            var messages = _mergeMessages(messages, newMessage);
        ...
        var _updatePatientReport = function (_data, _formName, _elem) {
            var form = _findForm(_data.patient_reports, _formName);
        ...
        $('#workflow-incoming').on('click', '.submit', function (_ev) {
            ...
            _updatePatientReport(data, 'V', $('#workflow-visits'));
            _updatePatientReport(data, 'D', $('#workflow-deliveries'));
        ...
        $('#workflow-outgoing').on('click', '.submit', function (_ev) {
            ...
            $('#workflow-outgoing .schedule').each(function() {
                ...
                $schedule.find('.message').each(function() {
                    ...
                    var contents = $message.find('[name=messages]').val();
                    var newContent = $message.find('[name=message]').val();
                    messages.push({
                        message: _mergeMessages(contents, newContent),
                ...
                schedules.push({
                    name: $schedule.find('[name=name]').val(),
                    start_from: $schedule.find('[name=start_from]').val(),
                    messages: messages
                });
            });
    ...
    var info = appinfo.getAppInfo.call(this, _req);
    var baseURL = require('duality/core').getBaseURL(_req);
    var workflow = workflows[_req.query.form];
    if (!workflow) {
    ...
    function _createForm (_forms, _name) {
        var form = _findForm(_forms, _name);
    ...
    function _createRegistration (_forms, _name) {
        var form = _findForm(info.registrations, 'R');
        var message = _findMessage(form.messages[0].message, info.locale);
        var validations = _.map(form.validations.list, function(_validation) {
            return {
                property: _validation.property,
                rule: _validation.rule,
        ...
        return {
            message: message,
            messages: JSON.stringify(form.messages[0].message),
            validations: validations
        };
    }
    ...
    var overview = templates.render(workflow.overview, _req, {});
    var incoming = templates.render(workflow.incoming, _req, {
        locale: info.locale,
        registration: _createRegistration(info),
        visit: _createForm(info.patient_reports, 'V'),
        delivery: _createForm(info.patient_reports, 'D')
    });
    var registration = _findForm(info.registrations, 'R');
    ...
    var schedules = _.map(schedules, function (_schedule) {
    ...
    var outgoing = templates.render(workflow.outgoing, _req, {
        locale: info.locale,
        schedules: schedules
    });
