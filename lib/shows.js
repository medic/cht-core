...
var updateControls = function(userCtx, form, dh_id) {
    var db = require('db').current(),
    ...
    $('.dropdown-menu.forms').closest('div').show();
    $('.dropdown-menu.forms').html(noneLink);
    $('#form-filter a.btn').html('Form' +
        (form ? ': <b>' + form + '</b>' : '') +
    ...
    $('.dropdown-menu.district-hospitals').html(noneLink);
    $('#district-filter a.btn').html('District' +
        (dh_name ? ': <b>' + dh_name + '</b>' : '') +
        ' <span class="caret"></span>');
    if(form) {
        q = {form: form};
...
exports.data_records = function(head, req) {
    events.once('afterResponse', function() {
        ...
        updateControls(req.userCtx, form, dh_id);
