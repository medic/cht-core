...
exports.configuration = function (_doc, _req) {
    ...
    var _formatTime = function (_info, _property) {
        var hours = _parseInt(info['schedule_' + _property + '_hours']);
        var minutes = _parseInt(info['schedule_' + _property + '_minutes']);
    ...
    return {
        ...
        content: templates.render('configuration.html', _req, {
            info: info,
            baseURL: baseURL,
            scheduleEvening: _formatTime(info, 'evening'),
            scheduleMorning: _formatTime(info, 'morning')
        })
    };
};
...
exports.workflow = function (_doc, _req) {
    ...
    var workflows = {
        'basic': {
            ...
            overview: 'workflow_basic_overview.html',
            incoming: 'workflow_basic_incoming.html',
            outgoing: 'workflow_basic_outgoing.html'
        }
    }
    ...
    events.once('afterResponse', function() {
        ...
        var _updateRegistration = function (_data) {
            ...
            registration.messages[0].message[0] = {
                content: elem.find('[name=message]').val(),
                locale: 'en'
            };
        };
        ...
        $('#workflow-outgoing').on('click', '.submit', function (_ev) {
            ...
            $('#workflow-outgoing .schedule').each(function() {
                ...
                $(this).find('.message').each(function() {
                    var offset = $(this).find('[name=offset_value]').val() + ' ' + 
                                 $(this).find('[name=offset_unit]').val();
                    messages.push({
                        message: [{
                            content: $(this).find('[name=message]').val(),
                            locale: 'en'
                        }],
                        group: $(this).find('[name=group]').val(),
                        offset: offset,
                        send_day: $(this).find('[name=send_day]').val(),
                        send_time: $(this).find('[name=send_time]').val(),
                        recipient: $(this).find('[name=recipient]').val()
                    });
                });
                schedules.push({
                    name: $(this).find('[name=name]').val(),
                    start_from: $(this).find('[name=start_from]').val(),
                    messages: messages
                });
            });
    ...
    var visit = _findForm(info.patient_reports, 'V');
    var delivery = _findForm(info.patient_reports, 'D');
    // TODO use selected language
    var registrationMessage = _findMessage(registration.messages[0].message, 'en');
    var visitSilence = _formatDate(visit.silence_for);
    var deliverySilence = _formatDate(delivery.silence_for);
    ...
    var schedules = _.map(schedules, function (_schedule) {
        var messages = _.map(_schedule.messages, function(_message) {
            return {
                ...
                content: _findMessage(_message.message, 'en')
            };
        });
        return {
            name: _schedule.name,
    ...
    return {
        ...
        content: templates.render('workflow.html', _req, {
            baseURL: baseURL,
            workflow: workflow,
            overview: overview,
            incoming: incoming,
