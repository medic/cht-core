...
var renderDownloadForms = function(err, data) {
    ...
    // param.
    //console.log(['hi cookie', req.cookie]);
    //console.log(['hi locale', req.cookie.kujua_locale]);
    ...
    db.getView('kujua-export', 'data_records_by_district', q, function(err, data) {
        ...
        if (req.query.dh_name) {
            $('#district-filter a.btn').html(
                'District: <b>'+req.query.dh_name+'</b> <span class="caret"></span>');
        } else {
            $('#district-filter a.btn').html(
        ...
        _.each(data.rows, function(dh, idx) {
            ...
            // special case for invalid records
            if(idx === data.rows.length - 1) {
                var t = db.stringifyQuery({
                            perPage: pagination.perPage,
                            limit: parseInt(pagination.perPage, 10) + 1,
                            dh_name: 'Invalid',
                            descending: true});
                var li = '<li><a href="?' + querystring.stringify(t)
                         + '"><i class="icon-exclamation-sign"></i> Invalid</a></li>';
                $('.dropdown-menu.district-hospitals').append(li);
            }
        });
        //$('.dropdown-menu.district-hospitals').append(
        //  '<li><a href="?descending=true&amp;dh_name=Invalid">Invalid</a></li>');
    });
};
