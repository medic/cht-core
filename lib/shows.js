...
var uploadFacilities = function(ev) {
    ...
    reader.onloadend = function(ev) {
        ...
        $('#facilities').html(
            templates.render(
                'facilities_restore.html', {}, {stat: '0/'+json.length}
            )
        );
        ...
        function updateProgress(complete, total) {
            ...
            $('#facilities-restore .stat span').text(complete + '/' + total);
            $('#facilities-restore .progress .bar').css('width', width+'%');
        }

        function saveRecord(doc, cb) {
        ...
        // look for record first and overwrite if specified
        ...
        function finishLoading(err) {
            console.log('finishLoading err',err);
...
var renderFacilitiesControls = function() {
    ...
    $('#facilities-controls .cancel').on('click', function() {
        $('#facilities-controls')[0].reset();
