...
var cache = {districts:[], health_centers:[]};
    ...
    events.once('afterResponse', function () {
        ...
        db.getView(ddoc, view, q, function (err, data) {
            ...
            var docs = _.map(data.rows, function (row) {
                var doc = row.doc;
                if (doc.parent) {
                    // pull out health_center name for use in spreadsheet
                    doc.hc_spreadsheet = doc.parent.name || '';
                }
                return doc;
            });
            $('#facilities').spreadsheet({
                columns: [
                    ...
                    {
                        label: 'Health Center',
                        property: ['hc_spreadsheet'],
                        type: 'string',
                        className: 'health_center', // added to td elements
                        embed: {
                            property: ['name'],
                            typeahead: {
                                source: cache.health_centers
                            }
                        }
                    }
                ],
                ...
                save: function (doc, callback) {
                    var q = {
                        key: ['health_center', doc.hc_spreadsheet],
                        include_docs: true
                    };
                    db.getView(ddoc, 'facilities', q, function (err, data) {
                        if (err) {
                            return callback(err);
                        }
                        if (!data.rows.length) {
                            /*return callback(new Error(
                                'No health center found with the name ' +
                                JSON.stringify(doc.hc_spreadsheet)
                            ));*/
                        }
                        doc.parent = data.rows[0].doc;
...
exports.health_centers = function (doc, req) {
    events.once('afterResponse', function () {
        ...
        db.getView(ddoc, view, q_dh, function (err, data) {
            ...
            cache.districts = [];
            _.each(data.rows, function(row) {
                cache.districts.push(row.key[1]);
            });
        });
        ...
        db.getView(ddoc, view, q, function (err, data) {
            ...
            var docs = _.map(data.rows, function (row) {
                var doc = row.doc;
                if (doc.parent) {
                    // pull out district name for use in spreadsheet
                    doc.dh_spreadsheet = doc.parent.name || '';
                }
                return doc;
            });
            $('#facilities').spreadsheet({
                columns: [
                    ...
                    {
                        label: 'District',
                        property: ['dh_spreadsheet'],
                        type: 'string',
                        className: 'district', // added to td elements
                        embed: {
                            typeahead: {
                                source: cache.districts,
                                items: 20
                            }
                        }
                    }
