...
var updateDistrictsControl = function(req) {
    ...
    // preserve query params except dh related
    delete q.dh_name;
    delete q.dh_id;

    // district filter trumps clinic filter
    delete q.cl_id;
    delete q.cl_name;
    ...
    if (cache[view] && cache[view].length > 0) {
        console.log('using cache',view);
        return update(cache[view]);
    }
...
var updateClinicsControl = function(req) {
    ...
    // preserve query params except cl related.  clinic filter trumps district
    // filter
    delete q.cl_name;
    delete q.cl_id;
    delete q.dh_id;
    ...
    db.getView(ddoc, view, view_q, function(err, data) {
        if (err) return alert(err);
        update(data.rows);
        cache[view] = data.rows;
    });
...
var updateControls = function(req) {
    ...
    //
    // gather query params. patient_id is exclusive.
    //
    var _q = {
        dh_id: dh_id,
        dh_name: dh_name,
    ...
    $('.page-header .controls').first().html(
        templates.render('data_records_controls.html', req, {query:req.query})
    ).show();
    ...
    // update the show all link for each filter
    var q1 = _.clone(_q);
    delete q1.form;
    $('.dropdown-menu.forms a').attr(
        'href', baseURL + '/data_records?' + querystring.stringify(q1)
    );
    ...
    // activate districts filter
    if (isAdmin) updateDistrictsControl(req);

    // activate clinics filter
    updateClinicsControl(req);
    ...
    db.getView(ddoc, view, {limit:1}, function(err, data) {
        if (err) return alert(err);
