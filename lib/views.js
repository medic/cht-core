...
exports.data_records = {
  map: function(doc) {
    ...
    if (doc.type === 'data_record') {
      clinicId = utils.getParentId(doc.contact, 'clinic');
      centerId = utils.getParentId(doc.contact, 'health_center');
      districtId = utils.getParentId(doc.contact, 'district_hospital');
...
exports.data_records_by_district = {
  map: function(doc) {
    if (doc.type === 'data_record') {
      var dh = require('views/lib/utils').getParent(doc.contact, 'district_hospital');
      if (dh) {
...
exports.delivery_reports_by_district_and_code = {
  map: function(doc) {
    if (doc.type === 'data_record' &&
      ...
      var dh = require('views/lib/utils').getParentId(doc.contact, 'district_hospital'),
...
exports.places_by_phone = {
  map: function (doc) {
    if (doc.contact && require('views/lib/utils').isPlaceType(doc)) {
...
exports.places_by_type_parent_id_name = {
  map: function (doc) {
    if (require('views/lib/utils').isPlaceType(doc)) {
...
exports.contacts_by_depth = {
  map: function(doc) {
    if (require('views/lib/utils').isContactType(doc)) {
      var parent = doc;
      var depth = 0;
      while (parent) {
        if (parent._id) {
